...
class Qwen3MoeKVCache:
    """Qwen3-MoE 专用的 KV 缓存包装器"""
    def __init__(self, model_instance):
        self._kvcache = model_instance.create_kv_cache()
        self.tokens = [0 for _ in range(40960)]  # max context length
        self.model_instance = model_instance
...
class Qwen3MoeInferTask:
    ...
    def __init__(self, tokens, position=0, temperature=1.0, topk=1, topp=1.0, 
        ...
        self.topk = topk
        self.topp = topp
        self.end_tokens = end_tokens or [151645]  # Qwen3-MoE 默认使用相同的 eos_token_id
        self._kv_cache = None
        self.pos = position
...
# Legacy classes for backward compatibility (keeping existing InferTask and KVCache)
class KVCache:
    def __init__(self, model_instance):
        self._kvcache = model_instance.create_kv_cache()
        self.tokens = [0 for _ in range(40960)]
        self.model_instance = model_instance

    def data(self):
        return self._kvcache

    def drop(self, model_instance=None):
        if self._kvcache:
            if model_instance:
                model_instance.drop_kv_cache(self._kvcache)
...
class InferTask:
    def __init__(self, task_id, tokens, max_tokens, temperature=1.0, topk=1, topp=1.0, end_tokens=None):
        self.id = task_id
        self.finish_reason = None
        self.tokens = tokens
        self.max_tokens = max_tokens
        ...
        self.topk = topk
        self.topp = topp
        self.end_tokens = end_tokens or [2]
        self._kv_cache = None
        self.pos = 0
