...
def load_model(model: nn.Module, path: str):
    ...
    if not model_files:
        raise FileNotFoundError(
            f"No model weights found in {path}. Looked for .safetensors and .bin files."
        )
        ...
        if is_safetensors:
            with safe_open(file_path, "pt", "cpu") as f:
                for weight_name in f.keys():
                    ...
                    _load_and_dispatch(
                        model, weight_name, tensor, packed_modules_mapping
                    )
        else:  # .bin format
    ...
    for packed_key, (target_name, shard_id) in packed_modules_mapping.items():
        if packed_key in weight_name:
            ...
            try:
                ...
                print(
                    f"Warning: Could not find parameter '{param_name}' for packed weight '{weight_name}'. Skipping."
                )
            is_packed = True
            break
