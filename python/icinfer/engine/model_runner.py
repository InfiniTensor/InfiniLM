...
from icinfer.engine.infer_task import InferTask, InferBatchedTask
...
class ModelRunner:
    ...
    def destroy(self):
        ...
        在程序退出时，安全地释放 C++ 侧的资源。
        """
        if hasattr(self, 'kv_cache_mock') and self.kv_cache_mock:
            print("drop_kv_cache")
            drop_kv_cache(self.model, self.kv_cache_mock)
            self.kv_cache_mock = None
    ...
    @torch.inference_mode()
    def batch_infer_one_round(self, tasks: List[InferTask], is_prefill: bool):
        output = (c_uint * len(tasks))()
    ...
    def run(self, seqs: list[Sequence], is_prefill: bool) -> list[int]:
        ...
        token_ids = self.batch_infer_one_round(tasks, is_prefill)
