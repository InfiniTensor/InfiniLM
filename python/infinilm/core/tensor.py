...
class Tensor:
    ...
    def __getattr__(self, name):
        ...
        if name == "shape":
            ...
            setattr(
                self,
                name,
                infinicore.device._from_core_device(getattr(self._underlying, name)),
            )
        else:
    ...
    def to_numpy(self):
        ...
        nbytes = tensor.numel() * np.dtype(np_dtype).itemsize
        raw = (ctypes.c_byte * nbytes).from_address(tensor.data_ptr())
        array = np.frombuffer(raw, dtype=np_dtype).reshape(tensor.shape)
...
def empty(size, *, dtype=None, device=None, pin_memory=False):
    return Tensor(_core.empty(size, dtype._underlying, device._underlying, pin_memory))
...
def zeros(size, *, dtype=None, device=None, pin_memory=False):
    return Tensor(_core.zeros(size, dtype._underlying, device._underlying, pin_memory))


def ones(size, *, dtype=None, device=None, pin_memory=False):
    return Tensor(_core.ones(size, dtype._underlying, device._underlying, pin_memory))


def from_blob(data_ptr, size, *, dtype=None, device=None):
    return Tensor(
        _core.from_blob(data_ptr, size, dtype._underlying, device._underlying)
    )


def strided_from_blob(data_ptr, size, strides, *, dtype=None, device=None):
    return Tensor(
        _core.strided_from_blob(
...
def from_torch(torch_tensor) -> Tensor:
    infini_type = to_core_dtype(torch_tensor.dtype)
...
def from_numpy(
    ...
    *,
    ...
    infini_type = dtype if dtype is not None else numpy_to_core_dtype(np_array.dtype)
