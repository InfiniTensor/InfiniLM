...
class DeviceEvent:
    ...
    def __init__(self, enable_timing=False, device=None):
        ...
        if device is None:
            # Use current device
            if flags == 0:
                self._underlying = _core.DeviceEvent()
            else:
                self._underlying = _core.DeviceEvent(flags)
        elif flags == 0:
            # Construct with device only
            self._underlying = _core.DeviceEvent(device._underlying)
        else:
            # Construct with both device and flags
            self._underlying = _core.DeviceEvent(device._underlying, flags)
    ...
    def device(self):
        ...
        return infinicore.device._from_core_device(self._underlying.device)
