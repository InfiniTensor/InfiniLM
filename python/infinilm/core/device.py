...
class device:
    ...
    def __getattr__(self, name):
        ...
        if name == "_underlying":
            setattr(self, name, device._to_core_device(self.type, self.index))
        else:
    ...
    @staticmethod
    def _to_core_device(type: str, index: int):
        return _python_2_core_dict[type][index]

    @staticmethod
    def _from_core_device(infinicore_device: _core.Device):
        return _core_2_python_dict[infinicore_device.type][infinicore_device.index]


_TORCH_DEVICE_MAP = {
    _core.Device.Type.CPU: "cpu",
    _core.Device.Type.NVIDIA: "cuda",
    _core.Device.Type.CAMBRICON: "mlu",
    _core.Device.Type.ASCEND: "npu",
    _core.Device.Type.METAX: "cuda",
    _core.Device.Type.MOORE: "musa",
    _core.Device.Type.ILUVATAR: "cuda",
    _core.Device.Type.KUNLUN: "cuda",
    _core.Device.Type.HYGON: "cuda",
    _core.Device.Type.QY: "cuda",
    _core.Device.Type.ALI: "cuda",
}
...
def _initialize_device_relationship(all_device_types, all_device_count):
    ...
    for infinicore_device_type, infinicore_device_count in zip(
        ...
        for infinicore_device_index in range(infinicore_device_count):
            ...
            infinicore_instance = _core.Device(
            ...
            if python_2_core_dict.get(python_device_type) is not None:
                python_2_core_dict[python_device_type].append(infinicore_instance)
            else:
                python_2_core_dict[python_device_type] = [infinicore_instance]

            if infinicore_device_type == _core.Device.Type.ASCEND:
                python_2_core_dict.setdefault("ascend", []).append(infinicore_instance)

    return infinicore_2_python_dict, python_2_core_dict


_all_device_types = tuple(_core.Device.Type.__members__.values())[:-1]
_all_device_count = tuple(
    _core.get_device_count(device) for device in _all_device_types
)

_core_2_python_dict, _python_2_core_dict = _initialize_device_relationship(
