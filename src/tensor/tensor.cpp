...
void Tensor::debug(size_t start_index, size_t num_elements, const std::string &label) const {
    ...
    std::cout << "Showing elements " << start_index << " to "
              << (start_index + actual_num_elements - 1)
    ...
    // coordinates -> memory offset (in elements)
    auto coordinates_to_offset = [&](const std::vector<size_t> &coords) -> size_t {
        size_t offset = 0;
        for (size_t d = 0; d < ndim; d++) {
            offset += coords[d] * stride[d];
        }
        return offset;
    };
    ...
    for (size_t i = start_index; i < start_index + actual_num_elements; i++) {
        ...
        size_t element_index = coordinates_to_offset(coords);
        size_t memory_offset = element_index * dsize(this->dtype());
