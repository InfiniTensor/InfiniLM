...
utils::Result<TopKInfo>
TopKInfo::create(infiniopTensorDescriptor_t input_desc,
    ...
    size_t workspace_size =
        needs_workspace
            ? num_tokens * num_experts * infiniop_dtype_size(dtype)
            : 0;
...
infiniStatus_t
Descriptor::create(infiniopHandle_t handle, Descriptor **desc_ptr,
                   infiniopTensorDescriptor_t input_desc,
                   infiniopTensorDescriptor_t output_val_desc,
                   infiniopTensorDescriptor_t output_ind_desc, int k) {
    auto result =
        TopKInfo::create(input_desc, output_val_desc, output_ind_desc, k);
    CHECK_RESULT(result);
...
size_t Descriptor::getWorkspaceSize() const {
    return _info.workspace_size();
}
...
infiniStatus_t Descriptor::calculate(const void *input, void *output_val,
    ...
    if (_info.workspace_size() > 0 && workspace == nullptr) {
        ...
        topkGatingSoftmaxKernelLauncher<int>(
            input, output_val, output_ind,
            nullptr, // token_expert_indices - not used in this context
        workspace, _info.num_tokens(), _info.num_experts(), _info.k(),
        (cudaStream_t)stream);
