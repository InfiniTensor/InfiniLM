#ifndef MIXTRAL_WEIGHT_HPP
#define MIXTRAL_WEIGHT_HPP

#include "mixtral_impl.hpp"

#include <cmath>
inline std::shared_ptr<Tensor> getInEmbd(
    MixtralMeta const *meta,
    MixtralWeights const *w) {
...
inline std::shared_ptr<Tensor> getAttnO(MixtralMeta const *meta,
                                        MixtralWeights const *w, size_t layer,
...
inline std::shared_ptr<Tensor> getSinTable(MixtralMeta const *meta) {
...
inline std::shared_ptr<Tensor> getCosTable(MixtralMeta const *meta) {
    auto half_dh = meta->dh / 2;
    auto unit = dsize(meta->dt_logits);
    void *table = std::malloc(meta->dctx * half_dh * unit);
