#ifndef TINYMIX_WEIGHT_HPP
#define TINYMIX_WEIGHT_HPP

#include "tinymix_impl.hpp"

#include <cmath>
inline std::shared_ptr<Tensor> getInEmbd(
    TinyMixMeta const *meta,
    TinyMixWeights const *w) {
...
inline std::shared_ptr<Tensor> getAttnO(TinyMixMeta const *meta,
                                        TinyMixWeights const *w, size_t layer,
...
inline std::shared_ptr<Tensor> getFFNGate(
    TinyMixMeta const *meta,
    TinyMixWeights const *w,
    size_t layer) {
    if (w->transpose_linear_weights != 0) {
        auto shape = std::vector<size_t>({meta->nexpert, meta->d});
        return Tensor::weight((char *)(w->ffn_gate[layer]), w->dt_mat, shape)
            ->permute({1, 0});
    } else {
        auto shape = std::vector<size_t>({meta->d, meta->nexpert});
        return Tensor::weight((char *)(w->ffn_gate[layer]), w->dt_mat, shape);
    }
}
...
inline std::shared_ptr<Tensor> getFFNGateUp(
    TinyMixMeta const *meta,
    TinyMixWeights const *w,
    size_t layer, size_t expert, size_t idev, size_t ndev) {
    auto di = meta->di;
    auto d = meta->d;
    size_t offset = idev * (2 * di / ndev) * d * dsize(w->dt_mat);
    if (w->transpose_linear_weights != 0) {
        auto shape = std::vector<size_t>({2 * di / ndev, d});
        return Tensor::weight((char *)(w->ffn_gate_up[layer][expert]) + offset,
                              w->dt_mat, shape)
            ->permute({1, 0});
    } else {
...
inline std::shared_ptr<Tensor> getFFNDown(
    ...
    if (w->transpose_linear_weights != 0) {
        ...
        return Tensor::weight((char *)(w->ffn_down[layer][expert]) + offset, w->dt_mat, shape)
            ->permute({1, 0});
    } else {
...
inline std::shared_ptr<Tensor> getSinTable(TinyMixMeta const *meta) {
...
inline std::shared_ptr<Tensor> getCosTable(TinyMixMeta const *meta) {
    auto half_dh = meta->dh / 2;
    auto unit = dsize(meta->dt_logits);
    void *table = std::malloc(meta->dctx * half_dh * unit);
