...
__C struct KVCache *createTinyMixKVCache(const TinyMixModel *model) {
...
__C struct KVCache *duplicateTinyMixKVCache(const TinyMixModel *model,
...
__C void dropTinyMixKVCache(TinyMixModel const *model, KVCache *kv_cache) {
    auto ndev = model->dev_resources.size();
    for (unsigned int idev = 0; idev < ndev; idev++) {
        RUN_INFINI(infinirtSetDevice(model->device, model->dev_ids[idev]));
        for (unsigned int layer = 0; layer < model->meta.nlayer; layer++) {
