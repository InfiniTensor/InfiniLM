...
class TestMarsBaseConfig(unittest.TestCase):
    ...
    def test_mars_uses_distinct_infinicore_device(self):
        config = self.make_config()
        self.assertEqual(config.get_device_str("mars"), "mars")

    def test_metax_uses_distinct_infinicore_device(self):
        config = self.make_config(device="metax")
        self.assertEqual(config.get_device_str("metax"), "metax")
