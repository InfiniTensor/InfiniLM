...
def _open_cap():
    try:
        ...
        try:
            ...
            return os.open(
                os.path.join(_HERE, ".stage3_log.tmp"),
...
def main():
    ...
    def build():
        return InferEngine(
            ...
            cache_config=PagedKVCacheConfig(
                a.num_blocks, a.block_size, max_batch_size=1
            ),
    ...
    # ------------------------------------------------------------- 1. 构造加载
    print(
        "\n== 1. paged 引擎构造 + 加载（blob %d / 稠密 %d）=="
        % (n_blob, len(table) - n_blob)
    )
    try:
        eng = build()
        ok, err = True, ""
    ...
    if not ok:
        return 1
    check(
        "has_mamba_cache 且 enable_paged_attn（GDN 模型只能走这条路）",
    ...
    print(
        "\n== 2-5. prefill：prompt=%d token（> kMaxDecodeM=%d）=="
        % (PREFILL_M, MAX_DECODE_M)
    )
    ...
    with capture() as cap:
        try:
            ...
            toks1, perr = (
                None,
                "%s: %s" % (type(e).__name__, str(e).strip().splitlines()[:1]),
            )
    log1 = cap.captured
    check(
        "prefill generate 走完 %d 步（M=%d 不再抛）" % (a.new_tokens, PREFILL_M),
        toks1 is not None,
        perr + "\n        日志尾部: " + log1[-500:],
    )
    if toks1 is None:
    ...
    if m:
        key, M, N, K, tid, row_bytes = (
            m.group(1),
            *[int(m.group(i)) for i in range(2, 7)],
        )
        print(
            "        %s — M=%d N=%d K=%d ggml_type=%d row_bytes=%d"
            % (key, M, N, K, tid, row_bytes)
        )
        ...
        check(
            "契约行 M=%d 等于 prompt 长度 %d（整批进 kernel）" % (M, PREFILL_M),
            M == PREFILL_M,
            "M=%d" % M,
        )
        check(
            "该批只能由 prefill 路径处理（M=%d > kMaxDecodeM=%d）" % (M, MAX_DECODE_M),
            M > MAX_DECODE_M,
            "M=%d" % M,
        )
        check(
            "契约行 row_bytes == (K/%d)*%d 自洽" % (bs, ts),
    ...
    check(
        "短 prompt 用例走完 %d 步（撤护栏未弄坏 gemv 路径）" % a.new_tokens,
        toks3 is not None,
        err3,
    )
    ...
    if a.count_blob_calls and not a.route_b_inner:
        ...
        check(
            "infiniopLinearGguf 命中 %s 次 == 步数 %s × blob %d = %s"
            % (n, steps, n_blob, expect),
            n is not None and n == expect,
            "实际 %s / 期望 %s\n        子进程输出尾部: %s" % (n, expect, tail[-600:]),
        )
