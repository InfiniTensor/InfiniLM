...
def main():
    ...
    check("quantization_config 在顶层且 quant_method=gguf", qc.get("quant_method") == "gguf",
          "qc keys=%s" % sorted(qc))
    ...
    check("表内 type id 都能从 gguf-py 查出 (block_size, type_size)：%s" % ids,
          all(i in bs_ts for i in ids), str([i for i in ids if i not in bs_ts]))
    ...
    print("  -> 类型表 %d 条：blob %d / 稠密 %d；产物 index %d 个张量；key_prefix='%s'"
          % (len(table), n_blob, len(table) - n_blob, len(weight_map), qc.get("key_prefix")))
    ...
    print("  -> 表键形态：%d/%d 条与产物张量名同名（新规则），其余 %d 条为相对名或前缀外键"
    ...
    dev_spec = infinicore.device(a.device) if ":" in a.device else infinicore.device(a.device, 0)
    try:
        eng = InferEngine(
    ...
    check("构造通过（= 被查询的 stem 全部恰好命中 1 个候选，且无 TP/bias 违规）", ok, err)
    if not ok:
    ...
    check("产物有、引擎不要（多键 -> strict=False 下静默丢权重）", not extra, str(extra[:12]))
    check("引擎要、产物没有（缺键 -> 保持随机初始化）", not missing, str(missing[:12]))
    check("键数一致（引擎 %d / 产物 %d）" % (len(keys), len(weight_map)),
    ...
    for fn in sorted(set(weight_map.values())):
        with safe_open(os.path.join(a.model_path, fn), framework="pt") as f:
            for k in f.keys():
                if k in sd_keys:
                    meta[k] = (f.get_slice(k).get_dtype(), list(f.get_slice(k).get_shape()))
    ...
    for k in sorted(sd_keys & set(meta)):
        ...
        if k.endswith("." + BLOB_SUFFIX):
            ...
            if "U8" not in str(eng_sd[k].dtype).upper() or meta[k][0] != "U8":
                bad_dtype.append("%s: 引擎 %s / 产物 %s" % (k, eng_sd[k].dtype, meta[k][0]))
            hits = table_hits(k)
            if len(hits) != 1:
                bad_shape.append("%s: 类型表命中 %d 个候选 %s（C++ 会抛或静默走稠密）"
                                 % (k, len(hits), hits[:4]))
                continue
            n_table_form["与张量名同名" if hits[0] == k else "相对名/归一后缀"] += 1
            _bs, ts = bs_ts[int(table[hits[0]])]
            if e_shape and ts and e_shape[-1] % ts:
                bad_shape.append("%s: row_bytes=%d 不是 type_size %d 的整数倍"
                                 % (k, e_shape[-1], ts))
    check("%d 个 blob 键在引擎侧与产物侧都是 U8" % n_blob_eng, not bad_dtype,
          str(bad_dtype[:6]))
    check("全部 %d 键 shape 逐字相等（blob 为 [out, row_bytes]）" % len(sd_keys),
          not bad_shape, str(bad_shape[:8]))
    print("  -> %d 个 blob 命中的表键形态：%s"
          % (n_blob_eng, ", ".join("%s x%d" % kv for kv in n_table_form.most_common())
             or "无"))
    ...
    if a.no_forward:
        ...
        def open_cap():
            try:
                ...
                return os.open(os.path.join(_HERE, ".stage2_log.tmp"),
        ...
        if not m:
            check("首个 blob Linear 进入 linear_gguf 并返回（未回落稠密）", False,
                  "python: %s\n        日志尾部: %s" % (" | ".join(msgs),
                                                            captured[-600:]))
        else:
            ...
            check("契约行自洽：M=%d 等于 prompt token 数 %d 且 row_bytes=%d == (K/%d)*%d"
