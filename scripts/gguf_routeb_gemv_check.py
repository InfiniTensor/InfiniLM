...
from gguf_routeb_blocks_ref import (  # noqa: E402
    BLOCK_SIZE,
    REF,
    TYPE_SIZE,
    TYPES,
...
PREFILL_MS = (
    "9,16,32,64,256,1024"  # 9 = prefill 下边界（§1.2 第 3 条含 16/32/64/256/1024）
)
...
def bf16_to_f32(bits):
    return (np.asarray(bits, np.uint16).astype(np.uint32) << np.uint32(16)).view(
        np.float32
    )
...
def pick_rows(art, t, want_rows, rng, which=0):
    ...
    if blocks_per_row * ts != row_bytes or blocks_per_row < 1:
        raise RuntimeError(
            "%s 的 row_bytes=%d 不是 block_size %d 的整数倍" % (name, row_bytes, ts)
        )
...
def check_type_one(binary, art, t, name, W, K, r0, Ms, rng, workdir):
    ...
    print(
        "  %s：%s（起始行 %d），%d 行 x K=%d，row_bytes=%d"
    ...
    for m in Ms:
        ...
        if not check(
            "%s M=%d：kernel 退出码 0" % (T_NAME.get(t, t), m),
            p.returncode == 0,
        ...
        check(
            "%s M=%d：走 %s 路径（与算子 calculate 的谓词一致）"
            % (T_NAME.get(t, t), m, want_path),
            got_path == want_path,
            "probe 报 path=%s" % got_path,
        )
        ...
        c = cos_sim(got, ref)
        check(
            "%s M=%d：cos_sim(kernel, 稠密 BF16 权重 @ x) > 0.999"
            % (T_NAME.get(t, t), m),
            c > 0.999,
            "cos_sim=%.8f" % c,
        )
        ...
        rel = (
            float(np.max(np.abs(dg[sig]) / np.abs(ref.astype(np.float64)[sig])))
            if sig.any()
            else 0.0
        )
        print(
            "        观测：cos_sim(kernel, 稠密 BF16 权重)=%.10f"
            "  cos_sim(kernel, 未舍入基准)=%.10f  bf16 逐位相同率=%.4f"
            "  max|Δ|=%.3e  max 相对偏差(|ref|≥最大幅值1%% 的子集)=%.3e  %s"
            % (
                c,
                cos_sim(got, ref_exact),
                same,
                absd,
                rel,
...
def check_rejections(binary, art, workdir):
    ...
    check(
        "未知 ggml type 10 被拒（rc=3，不启动 kernel）",
        p.returncode == 3,
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=200,
        help="每个张量取多少行权重（不是 64 的整数倍才能盖住 tile 余数）",
    )
    ap.add_argument(
        "--ms",
        default="1,8," + PREFILL_MS,
    ...
    if args.skip_build:
        ...
        try:
            bref.build_probe(
                PROBE_SRC, binary, args.nvcc, extra=["-I", GEMV_DIR, "-lcublas"]
            )
            check(
                "nvcc 编译 %s 通过（含两个 kernel 头 + cublas）"
                % os.path.basename(PROBE_SRC),
                True,
            )
        except Exception as exc:  # noqa: BLE001
            check(
                "nvcc 编译 %s 通过" % os.path.basename(PROBE_SRC),
                False,
                str(exc)[-2000:],
            )
            return 1
    ...
    print(
        ...
        % (bref._PASS, bref._FAIL, bref._SKIP)
    )
