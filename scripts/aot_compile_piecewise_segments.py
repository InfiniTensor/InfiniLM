...
"""AOTInductor export for many piecewise segments with one weight load per bucket."""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--layers", default="all", help="Layer indices: all, 0-31, or 0,1,5")
    parser.add_argument(
        "--valid-seq-len",
        type=int,
        default=0,
    ...
    from infinilm.compile.piecewise_segments import aot_compile_piecewise_segments_batch
    ...
    print(
        f"[aot_segments] segment={args.segment} bucket={args.bucket} "
    ...
    print(
        f"[aot_segments] PASS compiled={len(summaries)} skipped="
        f"{len(layer_indices) - len(summaries)} bucket={args.bucket}",
        flush=True,
    )
    ...
    if args.json_out:
        ...
        with open(args.json_out, "w", encoding="utf-8") as f:
            json.dump({"summaries": summaries, "bucket": args.bucket}, f, indent=2)
