...
class LLaVAForCauslLM:
    ...
    def generate(
        ...
        # HF transformers>=4.43: LlavaProcessor.apply_chat_template(tokenize=True) returns text-only
        # encodings (no pixel_values). Build prompt string first, then run the processor with images.
        from PIL import Image
            ...
            if isinstance(content, list):
                for it in content:
                    if isinstance(it, dict) and it.get("type") == "image":
                        url = it.get("url") or it.get("image")
                        if url:
                            image_urls.append(url)
        if len(image_urls) != 1:
            raise ValueError(f"Expected exactly 1 image in messages, got {len(image_urls)}")
        image = Image.open(image_urls[0]).convert("RGB")

        prompt = self.preprocessor.apply_chat_template(
            messages,
            add_generation_prompt=True,
            tokenize=False,
        )
        mm_inputs = self.preprocessor(text=prompt, images=image, return_tensors="pt")
        pixel_values = mm_inputs["pixel_values"]
        attention_mask = mm_inputs["attention_mask"]
        input_ids = mm_inputs["input_ids"]

        # HF LlavaProcessor keeps a single <image> token; the model expands it to 576 image tokens internally.
        # Our C++ path injects per-patch embeddings via override positions, so we expand here.
        expected_img_tokens = int(getattr(self.meta.vision_meta, "num_patches", 0))
        if expected_img_tokens > 1:
            image_token_index = int(self.config.get("image_token_index", 32000))
            ids0 = input_ids[0].tolist()
            mask0 = attention_mask[0].tolist()
            expanded_ids = []
            expanded_mask = []
            for tok, m in zip(ids0, mask0):
                if int(tok) == image_token_index:
                    expanded_ids.extend([image_token_index] * expected_img_tokens)
                    expanded_mask.extend([int(m)] * expected_img_tokens)
                else:
                    expanded_ids.append(int(tok))
                    expanded_mask.append(int(m))
            input_ids = torch.tensor([expanded_ids], dtype=input_ids.dtype)
            attention_mask = torch.tensor([expanded_mask], dtype=attention_mask.dtype)
