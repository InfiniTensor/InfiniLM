...
def main():
    ...
    with open(
        os.path.join(args.model_path, "model.safetensors.index.json"), encoding="utf-8"
    ) as f:
    ...
    for case in traced:
        ...
        result = {
            ...
            "bf16_weight_fp32_margin_llama_minus_infinilm": native_logits[0]
            - native_logits[1],
            "bf16_weight_fp32_winner": llama_token
            if native_logits[0] > native_logits[1]
            else infini_token,
            "hidden_shape": step["hidden_shape"],
        }
        results.append(result)
        print(
            "%-10s llama=%6d infini=%6d gguf_f32=%+.8f bf16w_f32=%+.8f winner=%d"
            % (
                result["case_id"],
                llama_token,
                infini_token,
    ...
    print(
        "RESULT gguf_f32_llama_wins=%d/%d bf16_weight_f32_llama_wins=%d/%d"
        % (
            sum(x["fp32_winner"] == x["llama_token"] for x in results),
            len(results),
            sum(x["bf16_weight_fp32_winner"] == x["llama_token"] for x in results),
            len(results),
        ),
        flush=True,
    )
