...
def main():
    ...
    ap.add_argument(
        "--stop-at-first-diff", action="store_true",
    ...
    ap.add_argument(
        "--operator-dump-layer", type=int,
        help="Override the per-case layer selected for generic operator dumps.")
    ap.add_argument("--gdn-dump-layer", type=int,
                     help="Enable GDN intermediate dumps for this layer.")
    ap.add_argument("--gdn-dump-seq-len", type=int, default=1,
    ...
    with open(args.compare, encoding="utf-8") as f:
        divergent = [x for x in json.load(f)["cases"]
                     if x["first_difference"] is not None]
    if args.case_id:
    ...
    if args.expected_results:
        with open(args.expected_results, encoding="utf-8") as f:
            current = json.load(f)
        expected_by_id = {
            x["id"]: [int(t) for t in x["runs"][0]["tokens"]]
            for x in current["cases"]
        }
    ...
    engine = InferEngine(
        model_path=args.model_path, device=infinicore.device("cuda:0"),
        distributed_config=DistConfig(1), cache_config=cache_config,
    ...
    for case_index, item in enumerate(divergent):
        ...
        for step in range(case_new_tokens):
            ...
            tensors = {
                ...
                "block_tables": infinicore.from_list([[kv_block]], dtype=infinicore.int32),
            ...
            cpp_input = engine._build_input(
                tensors["input_ids"], position_ids=tensors["position_ids"],
                past_kv_lengths=tensors["past_kv_lengths"],
                total_kv_lengths=tensors["total_kv_lengths"],
                input_offsets=tensors["input_offsets"], cu_seqlens=tensors["cu_seqlens"],
                block_tables=tensors["block_tables"], slot_mapping=tensors["slot_mapping"],
                mamba_init_state_indices=tensors["mamba_init_state_indices"],
                mamba_final_state_indices=tensors["mamba_final_state_indices"],
                sample_all_positions=False, temperature=0.0, top_k=1, top_p=1.0)
            output = _infinilm.InferEngine.forward(engine, cpp_input)
            token = int(np.asarray(infinicore.Tensor(output.output_ids).to_numpy()).reshape(-1)[0])
            ...
            candidates = [{"id": int(i), "logit": float(logits[i]),
                           "delta_from_top": float(logits[i] - top_logit)} for i in order]
            step_result = {"step": step, "selected": token,
                           "logits_shape": shape, "top_logits": candidates}
            if step == first_diff:
                ...
                if hidden_cpu.dtype == infinicore.bfloat16:
                    ...
                    hidden_bits = np.ctypeslib.as_array(
                        hidden_bits_type.from_address(hidden_cpu.data_ptr())).copy()
                    ...
                    hidden_values = np.ctypeslib.as_array(
                        (ctypes.c_float * hidden_cpu.numel()).from_address(
                            hidden_cpu.data_ptr())).copy()
                    step_result["hidden_dtype"] = "float32"
                    step_result["hidden_f32"] = [float(x) for x in hidden_values]
                else:
                    raise TypeError("expected BF16 or F32 hidden state, got %s" % hidden_cpu.dtype)
        ...
        if not stable and not args.allow_token_mismatch:
            raise RuntimeError("%s trace changed: %s != %s" % (case_id, generated, expected))
        results.append({"case_id": case_id, "tokens": generated, "steps": steps})
        print("%-10s tokens=%d stable=%s" % (case_id, len(generated), stable), flush=True)

    os.makedirs(os.path.dirname(os.path.abspath(args.out)), exist_ok=True)
    with open(args.out, "w", encoding="utf-8") as f:
        json.dump({"load_s": round(load_s, 4), "cases": results},
