...
def main():
    ...
    for item in divergent:
        ...
        body = {
            ...
            "n_predict": args.new_tokens, "temperature": 0.0,
            ...
            "return_tokens": True, "n_probs": args.n_probs,
            "stream": False, "samplers": ["top_k", "temperature"],
        }
        ...
        if tokens != expected:
            raise RuntimeError("%s rerun changed: %s != %s" % (case_id, tokens, expected))
        results.append({
            "case_id": case_id, "tokens": tokens,
            "completion_probabilities": raw.get("completion_probabilities", []),
        })
        print("%-10s tokens=%d probabilities=%d stable=%s" % (
            case_id, len(tokens), len(results[-1]["completion_probabilities"]), tokens == expected),
            flush=True)
