...
TOKENIZER_FILES = ("tokenizer_config.json", "chat_template.jinja", "generation_config.json",
                   "preprocessor_config.json", "video_preprocessor_config.json",
                   "special_tokens_map.json", "merges.txt", "vocab.json", "tokenizer.json")
...
def dense_float32(src: np.ndarray, type_name: str, chunk_rows: int) -> np.ndarray:
    ...
    for i in range(chunk_rows, rows, chunk_rows):
        out[i:i + chunk_rows] = np.asarray(dequantize(src[i:i + chunk_rows], q),
                                           dtype=np.float32)
    return out
...
def make_blob(e, t, dims, opt):
    ...
    if int(t.data.shape[-1]) != rb:
        raise ValueError("%s: 源行字节 %d != 映射表期望 %d"
    ...
    if int(arr.shape[0]) != n_out:
        raise ValueError("%s: 取段后 %d 行 != 映射表 %d" % (e.gguf, arr.shape[0], n_out))
    if M.needs_vperm(e):
...
def entry_float32(e, t, dims, opt) -> np.ndarray:
    ...
    if tuple(arr.shape) != want:
        if arr.size != prod(want):
            raise ValueError("%s: 变换后 shape %s != 映射表 %s"
...
def build(e, t, dims, opt, dense_all: bool):
    ...
    if dense_all and e.act_vperm and opt.vperm != "none":
        ...
        if in_dim != n_k * r * hd:
            raise ValueError("%s: in_dim %d != num_k_heads*num_v_per_k*head_dim = %d，无法按头分块置换列"
                             % (e.infinilm, in_dim, n_k * r * hd))
        # [out, in] 解释为 [out, r, n_k, hd]（tiled 序）-> 对调 1,2 轴 -> [out, n_k, r, hd]（grouped 序）-> flatten
        tens = tens.view(out_dim, r, n_k, hd).transpose(1, 2).contiguous().view(out_dim, in_dim)
...
def _as_dense(e):
    ...
    if v is None:
        ...
        v = M.Entry(e.infinilm, e.gguf, e.shape, False, tr, e.types, e.slices, e.vperm,
                    "dense-ref " + (e.note or ""))
        _DENSE_CACHE[key] = v
    return v
...
def check_dims(d: M.Dims) -> None:
    ...
    for f in _DIM_FIELDS:
        ...
        if isinstance(want, float) or isinstance(got, float):
            ...
            diff.append("%s: %r != %r" % (f.name, got, want))
    if diff:
        raise SystemExit("GGUF 元数据推导出的维度与 gguf_mapping.REAL 不符：%s\n"
                         "=> 先按新模型实测重做阶段 0，不要改打包器来迁就。" % diff)
    log("  rms_norm_eps：GGUF float32 %r -> config 写 HF 十进制 %r"
...
class ShardWriter:
    ...
    def flush(self) -> None:
        ...
        log("  写出 %s（%.2f GiB，%d 个张量）" % (fname, self.buf_bytes / _GiB, len(self.buf)))
        self.shards[-1] = fname
    ...
    def finish(self) -> None:
        ...
        with open(os.path.join(self.dir, "model.safetensors.index.json"), "w") as fp:
            json.dump({"metadata": {"total_size": self.total},
                       "weight_map": self.weight_map}, fp, indent=1, sort_keys=True)
...
def dense_bits_check(e, t, dims, opt, prod_t) -> bool:
    ...
    import torch
    if M.needs_vperm(e):
        return bool(np.array_equal(prod_t.view(torch.uint16).numpy(),
    ...
    for i in range(0, n, rows):
        blk = src[i:i + rows]
        arr = (np.asarray(blk, dtype=np.float32) if tn in UNQUANTIZED
...
def verify(out_dir: str, plan, tensors, dims, opt, sample) -> int:
    ...
    log("\n== 自检：重读产物 ==")
    bs_files = sorted(f for f in os.listdir(out_dir)
    ...
    if bad:
        fails += 1
        log("  FAIL shape/dtype 不符 %d 个：%s" % (len(bad),
            [(k, want[k][:2], got[k]) for k in sorted(bad)[:4]]))
    else:
    ...
    if qcfg.get("quant_method") != "gguf":
        fails += 1
        log("  FAIL config.json 顶层 quantization_config.quant_method != 'gguf'（或在 text_config 里）")
    ...
    for label, keys in (("类型表缺键", sorted(set(got) - set(table))),
    ...
    cats = [("blob 未置换", sel(lambda e: e.blob and not e.slices and not M.needs_vperm(e))),
            ("blob V 置换", sel(lambda e: e.blob and not e.slices and M.needs_vperm(e))),
            ("blob 融合切片", sel(lambda e: e.blob and e.slices)),
            ("bf16 反量化", sel(lambda e: not e.blob and not e.slices and not M.needs_vperm(e))),
    ...
    log("  抽样 %d 个：%s" % (len(picks), "全部 blob" if sample == "all" else
                              " ".join("%s=%s" % (c, len(v)) for c, v in cats)))
    for k in picks:
        shape, dt, e = want[k]
        ...
        if tuple(int(x) for x in prod_t.shape) != tuple(int(x) for x in ref.shape):
            ...
            checks.append(("BF16 位与 numpy RNE 一致",
                           dense_bits_check(e, tensors[e.gguf], dims, opt, prod_t)))
        ok = all(v for _, v in checks)
        fails += 0 if ok else 1
        log("  %s %-52s %-16s %s" % ("PASS" if ok else "FAIL", k,
                                     str(tuple(int(x) for x in prod_t.shape)),
                                     "，".join("%s=%s" % (n, "Y" if v else "N")
                                               for n, v in checks)))
    return fails
...
def main() -> int:
    ...
    ap.add_argument("--dense-iq", action=argparse.BooleanOptionalAction, default=True,
                    help="v1 把 5 个 IQ4_NL/IQ4_XS 稠密化（阶段 6 上了码本 kernel 后 --no-dense-iq）")
    ap.add_argument("--dense-embed", action=argparse.BooleanOptionalAction, default=True,
                    help="v1 恒为 True；--no-dense-embed 需要阶段 6 的 embedding kernel")
    ap.add_argument("--vperm", choices=("inv", "fwd", "none"), default="inv",
                    help="V 头 tiled->grouped 方向；阶段 4 A/B 用（§2.7）")
    ap.add_argument("--emit-dense-ref", metavar="PATH", default=None,
    ...
    if not a.dense_embed:
        raise SystemExit("--no-dense-embed 需要阶段 6 的 embedding / lm_head 原生 kernel，"
                         "v1 没有它们就只能稠密化（§2.4）")
    ...
    check_dims(dims)
    log("  维度与映射表 REAL 一致：%d 层，hidden=%d，vocab=%d"
    ...
    if a.layers is not None:
        if not 0 < a.layers < dims.n_layers:
            raise SystemExit("--layers 必须在 (0, %d) 之间，实际 %d"
                             % (dims.n_layers, a.layers))
        log("  --layers %d：num_hidden_layers %d -> %d，产物可加载"
    ...
    log("  blob %d 个 / 稠密化 %d 个 / 丢弃 MTP 前缀 %s"
    ...
    if a.dry_run:
        ...
        for e in plan:
            ...
            if e.blob:
                ...
                if int(t.data.shape[-1]) != rb:
                    raise ValueError("%s: 源行字节 %d != 期望 %d"
                                     % (e.gguf, int(t.data.shape[-1]), rb))
                if int(t.data.shape[0]) < n_out:
                    raise ValueError("%s: 源 %d 行 < 条目需 %d 行"
                                     % (e.gguf, t.data.shape[0], n_out))
                blob_bytes += n_out * rb
            else:
                n = prod(tuple(int(x) for x in e.shape))
                if not e.slices and prod(int(x) for x in t.shape) != n:
                    raise ValueError("%s: 源元素数 %s != 条目 shape %s"
                                     % (e.gguf, t.shape, tuple(e.shape)))
                dense_bytes += n * 2
        log("  PASS %d 个条目取向/字节数自洽：blob %.3f GiB + 稠密化 BF16 %.3f GiB"
            " = 产物应占 %.3f GiB"
            % (len(plan), blob_bytes / _GiB, dense_bytes / _GiB,
    ...
    log("  config.json：%d 个 ggml_types 键（quantization_config 在顶层）+ 激活 V 头置换规则 %d 条：%s"
        % (len(ggml_types), len(rules),
           " ".join("%s=%dx%dx%d" % (r["suffix"], r["num_k_heads"], r["num_v_per_k"],
                                     r["head_dim"]) for r in rules) or "无"))

    fails = export_tokenizer(reader, a.out, a.tokenizer_dir, dims)

    if not a.skip_pack:
        with open(os.path.join(a.out, "pack_report.json"), "w") as fp:
            json.dump({"gguf": os.path.abspath(a.gguf),
                       # 张量 data 区之和 != 文件大小（后者含元数据与对齐填充），
                       # 两者都记下来，免得日后拿这个数去对 stat 产生误会
                       "gguf_file_bytes": os.path.getsize(os.path.abspath(a.gguf)),
                       "gguf_tensor_data_bytes": sum(int(t.n_bytes) for t in reader.tensors),
                       "n_gguf_tensors": len(tensors),
                       "v1_dense_iq": bool(a.dense_iq), "vperm": a.vperm,
                       "n_entries": len(plan), "n_blob": len(blob),
                       "n_v1_exceptions": n_exc,
                       "blob_type_ids": sorted({TYPE_ID[opt.types[e.gguf]] for e in blob}),
                       "out_bytes": w.total, "shards": w.shards,
    ...
    if a.emit_dense_ref:
        ...
        for e in plan:
            ...
            wr.add(name, tens)
        wr.finish()
        ref_cfg = M.make_root_config(dims, {M.type_table_key(k): "dense_bf16"
                                           for k in ggml_types}, rules)
...
def export_tokenizer(reader, out_dir: str, tokenizer_dir: str, dims) -> int:
    ...
    if len(tokens) != dims.vocab:
        raise SystemExit("GGUF 词表 %d != config vocab_size %d，词表与 embedding 不同源"
                         % (len(tokens), dims.vocab))
    if model != "gpt2":
        log("  警告：tokenizer.ggml.model=%r 非 gpt2，vocab.json/merges.txt 写法需复核" % model)
    ...
    log("  词表来自 GGUF：vocab %d / merges %d（model=%s）" % (len(tokens), len(merges), model))

    have = os.listdir(tokenizer_dir) if tokenizer_dir and os.path.isdir(tokenizer_dir) else []
    if not have:
    ...
    if "tokenizer_config.json" not in copied + have:
        raise SystemExit("产物缺 tokenizer_config.json：既没从 %s 复制到，也没导出兜底"
                         % tokenizer_dir)
...
def check_tokenizer(out_dir: str, dims) -> int:
    ...
    try:
        ...
        log("  %s 分词器 %s vocab=%d 往返=%s" % ("PASS" if ok else "FAIL", cls, n,
                                                 tk.decode(ids) == s))
