...
def dims_from_text_config(tc):
    ...
    return M.Dims(
        ...
        n_q_heads=tc["num_attention_heads"],
        n_kv_heads=tc["num_key_value_heads"],
        head_dim=tc["head_dim"],
        ffn=tc["intermediate_size"],
        lin_k_heads=tc["linear_num_key_heads"],
        lin_v_heads=tc["linear_num_value_heads"],
        lin_k_dim=tc["linear_key_head_dim"],
        lin_v_dim=tc["linear_value_head_dim"],
...
def framework_side(engine_device):
    ...
    check(
        "探针 CFG 与 MINI 维度一致",
        dims_from_text_config(CFG["text_config"]) == M.MINI,
    ...
    eng = InferEngine(
        model_path=tmp,
        device=infinicore.device(engine_device, 0),
    ...
    check(
        "映射表无重复键（%d 条）" % len(plan),
        len(want) == len(plan),
        "%d vs %d" % (len(want), len(plan)),
    )

    missing = sorted(set(actual) - set(want))
    extra = sorted(set(want) - set(actual))
    check(
        "无缺键（框架要但映射表未提供 -> 会保持随机初始化）",
        not missing,
        str(missing[:12]),
    )
    check(
        "无多键（映射表提供但框架无此参数 -> strict=False 下静默丢）",
        not extra,
        str(extra[:12]),
    )

    bad = [
        (k, want[k], M.compress(actual[k]))
...
def dense_iq_bf16(plan, tensors, gguf_types, prod):
    ...
    return sum(
        prod(e.shape) * 2 - int(tensors[e.gguf].n_bytes)
        for e in plan
        if not e.blob
        and gguf_types.get(e.gguf) in M.V1_IQUANT_DENSE
...
def gguf_side(path):
    ...
    gguf_types = {
        n: TYPE_NAME.get(int(t.tensor_type), str(t.tensor_type))
    ...
    check(
        "shape 与 ne 反序全等（含 conv1d squeeze）", not bad_shape, str(bad_shape[:8])
    )
    check(
        "blob 条目行字节可整除且与 n_bytes 相符（%d 条）" % ok_blob,
        not bad_rows,
        str(bad_rows[:6]),
    )
    ...
    for name, es in shared.items():
        ...
        if not segs:
            ...
            segs[0][0] != 0
            or segs[-1][1] != n_out
            or any(segs[i][1] != segs[i + 1][0] for i in range(len(segs) - 1))
        ):
    ...
    print(
        "  -> 消费 %d 个 / 丢弃 %d 个（MTP blk.%d.*）/ 文件共 %d 个"
    ...
    print(
        "  按条目统计："
        + ", ".join("%s x%d" % (tn, c) for tn, c in all_types.most_common())
    )
    blob_types = {
        e.gguf: TYPE_NAME.get(int(tensors[e.gguf].tensor_type))
        for e in plan
        if e.blob and e.gguf in tensors
    }
    seen = collections.Counter(blob_types.values())
    print(
        "  blob 条目源类型："
        + ", ".join("%s x%d" % (tn, c) for tn, c in seen.most_common())
    )
    check(
        "阶段 3 v1 需实现的类型集合 = %s" % sorted(seen),
        set(seen) == set(M.NATIVE_BLOB_TYPES),
        "缺 %s / 多 %s"
        % (set(M.NATIVE_BLOB_TYPES) - set(seen), set(seen) - set(M.NATIVE_BLOB_TYPES)),
    )
    check(
        "IQ4_* 已被 v1 稠密化例外排除",
        not ({"IQ4_NL", "IQ4_XS"} & set(seen)),
        str(sorted(seen)),
    )
    giB = 2**30
    ...
    print(
        "  -> 文件 %.3f GiB = 逐字节 blob %.3f（%d 个） + 稠密化源 %.3f（%d 个）"
        " + MTP 丢弃 %.3f"
        % (total / giB, b / giB, len(blob_src), d_src / giB, len(dense_src), drop / giB)
    )
    ...
    print(
        "  -> v1 显存预算：blob %.3f + 稠密化 BF16 %.3f = %.3f GiB"
        % (b / giB, dense_bf16 / giB, budget)
    )
    check(
        "v1 权重预算 <= 24.0 GiB（单卡 5090 32.6 GiB 留 KV 余量）",
        budget <= 24.0,
        "%.3f GiB" % budget,
    )
    # 阶段 6 复利：IQ4 上原生 kernel 后再省；emb/lm_head 上 kernel 再省 2.51 GiB
    st6 = budget - dense_iq_bf16(plan, tensors, gguf_types, prod) / giB
    emb_out_blob = int(tensors["token_embd.weight"].n_bytes) + int(
        tensors["output.weight"].n_bytes
    )
    emb_out_bf16 = sum(
        prod(e.shape) * 2
        for e in plan
    ...
    print(
        "  -> 阶段 6：IQ4 原生 kernel %.3f GiB；再 emb/lm_head 原生 %.3f GiB"
        % (st6, st6b)
    )
    check(
        "阶段 6 预算单调下降",
        st6b < st6 < budget,
        "%.3f / %.3f / %.3f" % (st6b, st6, budget),
    )
    check(
        "阶段 6 目标态 <= 20.5 GiB（相对路线 A 的 26.6 GiB 权重）",
        st6b <= 20.5,
        "%.3f GiB" % st6b,
    )
...
def main():
    ...
    ap.add_argument(
        "--skip-min", action="store_true", help="跳过需要 infinilm 的框架侧检查"
    )
