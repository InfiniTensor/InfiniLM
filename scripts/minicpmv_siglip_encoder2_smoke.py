...
def main():
    ...
    model.infer_siglip_encoder(model_handle, 2, x.data_ptr(), seq_len, out_cpp.data_ptr())

    # Torch reference: layer0 -> layer1 -> post_layernorm
    ...
    l0 = SiglipEncoderLayer(cfg).to(dtype=torch_dt).eval()
    l1 = SiglipEncoderLayer(cfg).to(dtype=torch_dt).eval()
    l0.load_state_dict(
        {
    ...
    with torch.no_grad():
        h = x.unsqueeze(0)
        h = l0(h, attention_mask=None)[0]
        h = l1(h, attention_mask=None)[0]
        w = keepalive["post_ln_w"]
        b = keepalive["post_ln_b"]
        out_ref = F.layer_norm(h, (d,), w, b, 1e-6).squeeze(0)

    diff = (out_cpp.float() - out_ref.float()).abs()
    print("siglip encoder2 smoke:")
