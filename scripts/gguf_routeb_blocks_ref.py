...
def half_to_float(bits):
    ...
    out[norm] = (
        sign[norm]
        | ((exp[norm] + np.uint32(112)) << np.uint32(23))
        | (mant[norm] << np.uint32(13))
    )
    special = exp == 31
    out[special] = (
        sign[special] | np.uint32(0x7F800000) | (mant[special] << np.uint32(13))
    )
    sub = (exp == 0) & (mant != 0)
    if sub.any():
        ...
        for _ in range(11):
            ...
            e[need] -= 1
        out[sub] = (
            sign[sub].astype(np.int64) | ((e + 127) << 23) | ((m & 0x3FF) << 13)
        ).astype(np.uint32)
...
def check_half_decode():
    ...
    ok_nan = bool(
        np.array_equal(np.isnan(mine), np.isnan(truth))
    ...
    check(
        "numpy 参考的 half_to_float：有限值逐位相同（%d 个）+ NaN 仍为 NaN（%d 个）"
        % (int(finite.sum()), n_nan),
        ok and ok_nan,
        "不同 %d 个，首个 0x%04X：%s vs %s"
        % (
            neq.size,
            int(h[neq[0]]) if neq.size else 0,
    ...
    def __init__(self, path):
        ...
        for k, v in table.items():
            ...
            if n in self.table_norm and self.table_norm[n] != v:
                raise RuntimeError(
                    "归一化后 %s 撞键且 ggml type 不同（%d vs %d）"
        ...
        for tname in sorted(idx):
            ...
            if t not in TYPES:
                raise RuntimeError(
                    "%s 的 ggml type %d 不在路线 B 支持的 %s 里"
                    % (tname, t, list(TYPES))
                )
    ...
    def sample(self, t, want, rng):
        ...
        try:
            for name in names:
                ...
                if bpr * ts != row_bytes:
                    raise RuntimeError(
                        "%s 的 row_bytes=%d 不是 block_size %d 的整数倍"
                        % (name, row_bytes, ts)
                    )
                rows_needed = int(np.ceil(per_name / bpr))
                rows = np.sort(
                    rng.choice(nrows, size=min(rows_needed, nrows), replace=False)
                )
...
def edge_blocks(t, rng, n_random=2048):
    ...
    ts = TYPE_SIZE[t]
    rows = [
        np.zeros(ts, np.uint8),
        np.full(ts, 0xFF, np.uint8),
        np.full(ts, 0x00, np.uint8),
        np.full(ts, 0x01, np.uint8),
    ]
...
def build_probe(src, out, compiler, extra=()):
    cmd = [compiler, "-O2", "-std=c++17", "-I", HEADER_DIR, src, "-o", out] + list(
        extra
    )
...
def run_probe(binary, t, blocks, workdir, tag):
    ...
    p = subprocess.run(
        [binary, str(t), str(blocks.shape[0]), inbin, f32bin, bf16bin],
        capture_output=True,
        text=True,
    )
    if p.returncode != 0:
...
def main():
    ...
    ap.add_argument(
        "--model-path", default="/home/liuxd/models/Qwen3.8-27B-GGUF-native-mini8"
    )
    ap.add_argument(
        "--blocks", type=int, default=20000, help="每种类型取多少真实 block"
    )
    ...
    print(
        ...
        % (
            args.model_path,
            os.path.join(HEADER_DIR, "ggml_blocks.h"),
            args.workdir,
            args.blocks,
        )
    )
    ...
    print(
        "产物 blob 张量 %d 个（key_prefix=%r），按类型：%s"
        % (n_blob_total, art.prefix, {t: len(art.type_names(t)) for t in TYPES})
    )
    check(
        "类型表 blob 条目与产物 weight_bytes 张量双向对平（表 %d / 张量 %d，孤儿 %d，"
        "匹配形态 %s）"
        % (
            art.n_table_blob,
            n_blob_total,
            len(art.orphan_table_keys),
            dict(art.match_form),
        ),
    ...
    print("\n[1] 编译 probe driver")
    host_bin = build_probe(
        PROBE_CPP, os.path.join(args.workdir, "blocks_probe_host"), args.cxx
    )
    ...
    if args.no_cuda:
        ...
        try:
            dev_bin = build_probe(
                PROBE_CU,
                os.path.join(args.workdir, "blocks_probe_cuda"),
                args.nvcc,
                extra=["-x", "cu"],
            )
    ...
    for t in TYPES:
        ...
        if not check(
            "%s 采到 %d 个真实 block（目标 %d，覆盖 %d 个张量）"
        ...
        check(
            "%s numpy 参考 vs gguf-py（%d block 逐位相同）" % (name, blocks.shape[0]),
        ...
        try:
            ...
            continue
        check(
            "%s 头的 block_elems 与 GGML_QUANT_SIZES 一致（%d == %d）"
            % (name, elems, BLOCK_SIZE[t]),
            elems == BLOCK_SIZE[t],
        )
        n_bad, n_diff, maxabs, first = bitwise_diff(h_f32, ref)
        check(
            "%s 头(host) fp32 vs numpy 参考（%d 元素逐位相同）" % (name, h_f32.size),
            n_diff == 0 and n_bad == 0,
        ...
        check(
            "%s 头(host) bf16 vs numpy RNE 舍入" % name,
            same_own,
        ...
        try:
            ...
            tv = (
                torch.from_numpy(np.ascontiguousarray(ref))
                .to(torch.bfloat16)
                .view(torch.uint16)
                .numpy()
            )
            check(
                "%s 头(host) bf16 vs torch .to(bfloat16)" % name,
                np.array_equal(h_bf16, tv),
            )
        except Exception as e:
        ...
        if dev_bin is not None:
            try:
                ...
                check(
                    "%s 头(cuda) fp32 vs 头(host) 逐位相同" % name,
                    np.array_equal(d_f32.view(np.uint32), h_f32.view(np.uint32)),
                )
                check(
                    "%s 头(cuda) bf16 vs 头(host) 逐位相同" % name,
        ...
        check(
            "%s 边界块（%d 个）numpy vs gguf-py 逐位相同" % (name, eb.shape[0]),
            n_diff_e == 0,
            "差异 %d，非有限 %d，max|Δ|=%.3g，首个：%s"
            % (n_diff_e, n_bad_e, maxabs_e, first_e),
        )
        check(
            "%s 边界块（%d 个）头(host) vs numpy 逐位相同" % (name, eb.shape[0]),
            n_diff_h == 0,
            "差异 %d，max|Δ|=%.3g，首个：%s" % (n_diff_h, maxabs_h, first_h),
        )
    ...
    for t in TYPES:
        ...
        check(
            "%s 头(host) vs numpy 参考：half 全域扫描 %d block 逐位相同"
            % (name, sb.shape[0]),
            n_diff == 0,
    ...
    check(
        "头对 ggml type 10（TQ1_0，非本头范围）返回拒绝",
