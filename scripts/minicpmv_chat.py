...
def main():
    ...
    if args.kv_compress and int(getattr(args, "kv_compress_image_len", 0)) == 0 and int(bounds.numel()) > 0:
        # Treat everything before the end of the last image feature span as image KV prefix.
        # This mirrors scripts/llava.py's approximate strategy.
        args.kv_compress_image_len = int(bounds[:, 1].max().item())
        if debug:
            print("INFO: auto kv_compress_image_len:", int(args.kv_compress_image_len))
