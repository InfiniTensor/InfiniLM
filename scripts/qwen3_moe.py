...
class Qwen3MoEForCauslLM:
    def __init__(
        ...
        self.qwen3moe_model = Qwen3MoEModel()
        ...
        self.model_instance = self.qwen3moe_model.create_model(
            byref(self.meta),
            byref(self.weights),
            device,
            ndev,
    ...
    def create_kv_cache(self):
        return self.qwen3moe_model.create_kv_cache(
            self.meta.nlayer,
            self.meta.dctx,
            self.meta.nkvh,
            self.meta.dh,
    ...
    def drop_kv_cache(self, kv_cache):
        self.qwen3moe_model.drop_kv_cache(kv_cache)

    def batch_infer_one_round(self, tasks: List[InferTask]):
        output = (c_uint * len(tasks))()
        batch_inputs = BatchedTask(tasks)
        self.qwen3moe_model.infer_batch(
            self.model_instance,
    ...
    def perplexity(self, test_sequences: List[Sequence[int]], batch_size=10):
        ...
        for i in range(0, len(test_sequences), batch_size):
            ...
            self.qwen3moe_model.forward_batch(
                self.model_instance,
                batch_inputs.tokens,
                batch_inputs.ntok,
    ...
    def destroy_model_instance(self):
        self.qwen3moe_model.destroy_model(self.model_instance)
...
def test():
    if len(sys.argv) < 3:
        print(
            "Usage: python qwen3_moe.py [--cpu | --nvidia| --cambricon | --ascend | --metax | --moore | --iluvatar | --kunlun | --hygon] <path/to/model_dir> [n_device]"
        )
        sys.exit(1)
