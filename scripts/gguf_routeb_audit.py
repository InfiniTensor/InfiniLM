...
def _rows_to_blocks(blob: np.ndarray, type_size: int) -> np.ndarray:
    ...
    assert row_bytes % type_size == 0, (
        f"row_bytes={row_bytes} 不是 type_size={type_size} 的整数倍"
    )
...
def decode_q4_k(blob: np.ndarray, n_cols: int) -> np.ndarray:
    ...
    qs = b[:, 16:ts]
    q = (
        ((qs[:, K_QS_BYTE] >> K_QS_SHIFT.astype(np.uint8)) & np.uint8(0x0F))
        .reshape(b.shape[0], 8, 32)
        .astype(np.float32)
    )
...
def decode_q6_k(blob: np.ndarray, n_cols: int) -> np.ndarray:
    ...
    q = (
        ((lo | (hi << np.uint8(4))).astype(np.int16) - 32)
        .reshape(n, 16, 16)
        .astype(np.float32)
    )
...
def section_a(reader) -> None:
    ...
    check(
        "F32 张量的 data 也是 C 序 [shape[1], shape[0]]（= HF 取向，打包器不转置）",
    ...
    check(
        "1-D norm 保持 dtype=float32且长度 = hidden",
...
def section_b() -> None:
    ...
    print(
        f"  {'type':8s} {'bs':>4s} {'ts':>4s} {'in':>7s} {'row_bytes':>10s} "
        f"{'行对齐':>7s} {'块起始对齐':>10s}"
    )
    worst_row, worst_block = 16, 2
    for name, bs, ts, n_in, rb, a, ab in facts:
        print(
            f"  {name:8s} {bs:4d} {ts:4d} {n_in:7d} {rb:10d} {str(a) + 'B':>7s} {str(ab) + 'B':>10s}"
        )
        worst_row = min(worst_row, a)
        worst_block = min(worst_block, ab)
    check(
        "块起始地址仅保证 2B 对齐（Q6_K=210B / Q8_0=34B 非 4 倍数）",
        worst_block == 2,
        f"min_block_align={worst_block}B",
    )
    check(
        "Q6_K 在 in=5120/17408 时行 stride 仅 8B 对齐",
...
def section_c() -> None:
    ...
    check(
        "reorder_v 是整头搬运（每个 head 的 hd 行连续不被打散）",
        all(
            np.array_equal(
                tiled[i * hd : (i + 1) * hd],
                grouped[
                    ((i % n_k) * n_v_per_k + i // n_k) * hd : (
                        (i % n_k) * n_v_per_k + i // n_k
                    )
                    * hd
                    + hd
                ],
    ...
    check(
        "tiled 序直接喂给 `value_head_idx / value_heads_per_key_head` 会错配 k 头",
        k_grouped != k_tiled,
    ...
    check(
        "逆变换后槽位归属恢复 grouped 语义",
        np.array_equal(
            reorder_v_inverse(
                np.array(
                    [k * n_v_per_k + v for v in range(n_v_per_k) for k in range(n_k)]
                ),
                n_k,
                n_v_per_k,
                1,
    ...
    check(
        "in_proj_a/b・A_log・dt_bias 的 head_dim=1 退化形式（逐元素置换）同样自等",
        np.array_equal(
            reorder_v_inverse(
                reorder_v(np.arange(n_v), n_k, n_v_per_k, 1), n_k, n_v_per_k, 1
            ),
            np.arange(n_v),
        ),
    )
    check(
        "多维情形（如 conv1d 的 [channels, 1, kernel]）仅置换头维、尾部轴不动",
        np.array_equal(
            reorder_v_inverse(
                reorder_v(grouped[:, :1], n_k, n_v_per_k, hd), n_k, n_v_per_k, hd
            ),
            grouped[:, :1],
        ),
    )
    ...
    check(
        "量化 blob 的行置换 == 字节整行置换（无需重新量化）",
        np.array_equal(blob[perm], np.ascontiguousarray(blob)[perm]),
    )
    print(
        "  -> 结论：整行置换可字节级完成；ssm_out 的列(in 维)置换不可，改用运行时激活 gather。"
    )
...
def section_d(reader) -> None:
    ...
    check(
        "主模型层数 64（block_count 含 1 个 MTP 层）",
        n_layer_gguf == n_main + 1,
        f"block_count={n_layer_gguf}",
    )
    check(
        "full attention 层 = 3,7,...,63 共 16 层",
    ...
    for name, want in shapes.items():
        probe = {
            ...
            "attn_k": f"blk.{full[0]}.",
            "attn_v": f"blk.{full[0]}.",
            "attn_output": f"blk.{full[0]}.",
            "attn_qkv": f"blk.{gdn[0]}.",
            "attn_gate": f"blk.{gdn[0]}.",
            "ssm_out": f"blk.{gdn[0]}.",
            "ssm_conv1d": f"blk.{gdn[0]}.",
            "ffn_gate": f"blk.{0}.",
            "ffn_up": f"blk.{0}.",
            "ffn_down": f"blk.{0}.",
        }[name]
    ...
    n_q, hd_q = 24, 256
    check(
        "attn_q 行数 = n_q*head*2（q 与 gate 每头交错）",
        shapes["attn_q"][1] == n_q * hd_q * 2,
    )
    check(
        "Qwen35FusedQKVLinear 期望 out = 12288 + 1024 + 1024 = 14336",
        12288 + 1024 + 1024 == 14336,
    )
    check(
        "GDN in_proj_qkv 行数 = q2048 + k2048 + v6144 = 10240",
        2048 + 2048 + 6144 == shapes["attn_qkv"][1],
    )
    check(
        "conv 通道 = 2*head_k*n_k + head_v*n_v = 10240",
        2 * 128 * 16 + 128 * 48 == 10240,
    )
    mtp = [n for n in tensors if n.startswith("blk.64.")]
    nextn = [n for n in tensors if ".nextn." in n]
    check(
        "MTP 丢弃规则 = 整块 blk.64.*（不止 .nextn.*，包含完整一层）",
    ...
    check(
        "块号集合 = 0..64（64 主层 + 1 MTP 层，无其它残留）",
        max_blk == 64
        and len({int(n.split(".")[1]) for n in tensors if n.startswith("blk.")}) == 65,
        f"max_blk={max_blk}",
    )
...
def section_f(reader) -> None:
    ...
    M.apply_v1_exceptions(
        plan, {n: _tn[int(t.tensor_type)] for n, t in tensors.items()}
    )
    ...
    for n in blob_src:
        bucket[f"U8 blob {QType(int(tensors[n].tensor_type)).name}"] += int(
            tensors[n].n_bytes
        )
        cnt[f"U8 blob {QType(int(tensors[n].tensor_type)).name}"] += 1
    d_emb = sum(
        int(np.prod(e.shape)) * 2
        for e in dense_e
    ...
    bucket["丢弃(MTP)"] = sum(
        int(t.n_bytes) for n, t in tensors.items() if n.startswith("blk.64.")
    )
    ...
    check(
        "v1 权重合计 ≤ 24.0 GiB（单卡 32607 MiB 可容纳权重+KV+激活）",
        total <= 24.0,
        f"total={total:.3f} GiB",
    )
    check(
        "MTP 丢弃量 < 0.4 GiB（不影响预算）",
        bucket["丢弃(MTP)"] / GiB < 0.4,
        f"{bucket['丢弃(MTP)'] / GiB:.3f} GiB",
    )
    check(
        "v1 blob 桶恰好只含阶段 3 实现的 4 种类型",
    ...
    check(
        "token_embd / output 也是量化的（Q6_K / Q8_0），v1 必须反量化它们",
        int(emb.tensor_type) == int(QType.Q6_K)
        and int(out.tensor_type) == int(QType.Q8_0),
        f"emb={emb.tensor_type} out={out.tensor_type}",
    )
    check(
        "emb/output 均为 [hidden, vocab] 且 vocab 与元数据一致",
    ...
    print(
        "  -> 阶段 6 可选项：emb 走 Q6_K 行 gather-dequant、lm_head 走 linear_gguf(Q8_0)，"
...
def section_e(reader) -> None:
    ...
    check(
        "partial rotary: dimension_count=64, head_dim=256 -> factor 0.25",
        dim_cnt == 64 and dim_cnt * 4 == head_dim,
        f"dimension_count={dim_cnt}",
    )
    check(
        "mrope sections [11,11,10,0] 之和 = 32 = dimension_count/2",
        sum(rope_secs) == dim_cnt // 2,
        f"sections={rope_secs}",
    )
    check("rope_theta = 1e7", base == 1e7, f"base={base}")
    check(
        "mtp 层数声明为 1（与 block_count=65 = 64+1 一致）",
    ...
    check(
        "ssm: inner 6144 / group 16 / state 128 / time_step_rank 48 / conv 4",
        (inner, n_k, st, dt, int(kv("ssm.conv_kernel"))) == (6144, 16, 128, 48, 4),
    )
    check(
        "value heads = inner/state = 48 = time_step_rank（两路推导一致）",
        inner // st == dt == 48,
        f"inner/state={inner // st} time_step_rank={dt}",
    )
    check("num_k_heads * state = 2048 = q/k 段长度", n_k * st == 2048)
    vocab = len(gguf_meta(reader, "tokenizer.ggml.tokens"))
    print(
        f"  arch={gguf_meta(reader, 'architecture')[0]!r}  "
...
def main() -> int:
    ...
    ap.add_argument(
        "--gguf", default="/home/liuxd/models/Qwen3.8-27B-GGUF/Qwen3.8-27B-UD-Q6_K.gguf"
    )
    args = ap.parse_args()
