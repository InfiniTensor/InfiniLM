...
for i in range(64):
    full = (i + 1) % 4 == 0
    names = ([f"blk.{i}.attn_q.weight", f"blk.{i}.attn_k.weight",
              f"blk.{i}.attn_v.weight", f"blk.{i}.attn_output.weight"] if full
             else [f"blk.{i}.attn_qkv.weight", f"blk.{i}.attn_gate.weight",
                   f"blk.{i}.ssm_out.weight"])
    names += [f"blk.{i}.ffn_gate.weight", f"blk.{i}.ffn_up.weight", f"blk.{i}.ffn_down.weight"]
    for n in names:
...
bad = [(i, [tn(f"blk.{i}.attn_{x}.weight") for x in ("q", "k", "v")])
...
for i in (0, 1, 2, 4, 62):
    print("  ", i, {s: tn(f"blk.{i}.{s}.weight") for s in
                    ("attn_qkv", "attn_gate", "ssm_out", "ffn_gate", "ffn_down")})
