...
def main() -> int:
    ...
    for item in divergent:
        ...
        body = {
            ...
            "frequency_penalty": 0.0, "seed": 1, "ignore_eos": True,
            "cache_prompt": False, "return_tokens": True,
            "n_probs": args.top_k, "stream": False,
        ...
        candidate_table = [{
            "id": token_id, "llama_logprob": llama_map.get(token_id),
        ...
        selected_logits = {
            ...
            "infini_margin_selected_minus_llama":
                float(logits[infini_selected] - logits[llama_selected]),
            "llama_margin_selected_minus_infini":
                float(llama_map[llama_selected] - llama_map.get(infini_selected, float("nan"))),
        }
        result = {
            "case_id": case_id, "first_difference": first_diff,
            "prefix_length": len(prefix), "llama_selected": llama_selected,
            "infinilm_selected": infini_selected,
            "llama_top_logprobs": llama_probs, "infinilm_top_logits": infini_top,
            "selected_pair": selected_logits, "candidate_table": candidate_table,
            "infinilm_logits_shape": shape,
            "infinilm_logits_finite": bool(np.isfinite(logits).all()),
        }
        results.append(result)
        print("%-10s diff=%2d llama=%6d infini=%6d llama_margin=%+.6f infini_margin=%+.6f" % (
            case_id, first_diff, llama_selected, infini_selected,
    ...
    print("RESULT cases=%d finite=%s load=%.3fs" % (
