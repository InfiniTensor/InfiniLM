...
void save_tensor_debug(const std::shared_ptr<Tensor> &tensor,
  ...
  if (has_issues && g_debug_enabled) {
    printf("WARNING: Tensor %s has data quality issues - check %s\n",
...
void createQwen3DeviceResource(DeviceQwen3Resource *rsrc, const Qwen3Meta *meta,
  ...
  // 增加内存池大小
  auto memory_pool =
      std::make_shared<MemoryPool>(512 * 1024 * 2048); // 从128MB增加到512MB

  if (g_debug_enabled && idev == 0) {
    printf("Memory pool size increased to 512MB\n");
  }
 ...
 * Supports tensor parallelism where each device processes a slice of model
 * parameters.
 */
void inferQwen3DeviceBatch(const Qwen3Meta &meta, DeviceQwen3Resource &rsrc,
  ...
  if (dt_logits != dt_logits_corrected) {
    if (g_debug_enabled && idev == 0) {
      ...
      oss << "WARNING: Corrected dt_logits from " << dt_logits << " to "
          << dt_logits_corrected << " to match weights\n";
      if (g_debug_enabled)
  ...
  if (rsrc.device == INFINI_DEVICE_CPU) {
    pos_ids_buf =
        Tensor::weight(batch_pos_ids.data(), INFINI_DTYPE_U32, {ntok});
  } else {
  ...
  for (uint32_t i = 0; i < ntok; i++) {
    ...
    if (g_debug_enabled && idev == 0 && i == 0) {
      std::ostringstream oss;
      oss << "DEBUG: Copying embedding for token " << tokens[i]
          << " to position " << i << "\n"
          << "  Source: " << rsrc.w_in_embd->data(tokens[i] * d)
          << " (offset: " << (tokens[i] * d) << ")\n"
          << "  Dest: " << logits_in->data(i * d) << " (offset: " << (i * d)
          << ")\n"
          << "  Size: " << (dsize(dt_logits) * d) << " bytes\n";
      if (g_debug_enabled)
        printf("%s", oss.str().c_str());
    }
  ...
  if (g_debug_enabled && idev == 0) {
    std::ostringstream oss;
    oss << "DEBUG: After embedding lookup, checking first token result:\n";
    ...
    oss << "  First 10 values after copy: ";
    ...
    oss << "  Comparing with original embedding values:\n"
        << "Original: 0x2660 0x9da8 0xac50 0xa518 0xa8c0\n"
        << "  Copied:   ";
  ...
  RUN_INFINI(infiniopCreateRoPEDescriptor(
      ...
      q_rope_single_head_3d->desc(), pos_ids_buf->desc(),
      rsrc.sin_table->desc(), rsrc.cos_table->desc()));

  RUN_INFINI(infiniopCreateRoPEDescriptor(
      rsrc.handle, &desc_rope_k_single, k_rope_single_head_3d->desc(),
      k_rope_single_head_3d->desc(), pos_ids_buf->desc(),
      rsrc.sin_table->desc(), rsrc.cos_table->desc()));
  ...
  auto desc_attn_v_rearranges =
      std::vector<infiniopRearrangeDescriptor_t>(nreq);
  ...
  for (uint32_t req = 0; req < nreq; req++) {
    ...
    // 步骤2：将2D切片分割成3D多头格式
    auto o =
        o_2d->dimSplit(1, {nh, dh}); // [seq_len, nh*dh] -> [seq_len, nh, dh]
    auto q = q_2d->dimSplit(
        1,
        {nh, dh}); // [seq_len, nh*dh] -> [seq_len, nh, dh] - 从归一化后的Q切片
     ...
     *
     * 重新塑造查询以实现高效的 GQA 计算：
     * q：[seq_len, nh, dh] -> [seq_len0, nkvh1, ngroup2, dh3] -> [nkvh1,
     * ngroup2, seq_len0, dh3] 此布局允许每个 KV 头关注多个查询头（每个 KV 头
     * ngroup 个查询头）
     */
     ...
     * [nkvh, ngroup, seq_len, dh] -> [seq_len, nkvh, ngroup, dh] -> [seq_len,
     * nh, dh]
     */
    ...
    auto qk =
        TensorDesc::create(dt_logits, {nkvh, ngroup * seq_len, total_len});
    ...
    RUN_INFINI(
        infiniopCreateGemmDescriptor(rsrc.handle, &desc_attn_v_gemms[req],
                                     q_t->desc(), qk->desc(), full_v->desc()));
    RUN_INFINI(
        infiniopGetGemmWorkspaceSize(desc_attn_v_gemms[req], &temp_size));
    workspace_size = std::max(workspace_size, temp_size);
    ...
    RUN_INFINI(infiniopGetCausalSoftmaxWorkspaceSize(desc_qk_softmaxs[req],
                                                     &temp_size));
    workspace_size = std::max(workspace_size, temp_size);
  ...
  RUN_INFINI(infiniopCreateRMSNormDescriptor(
      ...
      logits_out->slice(0, 0, 1)->desc(), rsrc.w_out_norm->desc(),
      meta.epsilon));
  ...
  RUN_INFINI(infiniopCreateGemmDescriptor(
      ...
      logits_out->slice(0, 0, nreq)->desc(), rsrc.w_out_embd->desc()));
  ...
  RUN_INFINI(infiniopCreateRandomSampleDescriptor(
      ...
      TensorDesc::create(INFINI_DTYPE_I64, {}, {})
          ->desc(), // 输出：标量 token ID
      TensorDesc::create(dt_logits, {dvoc}, {1})
          ->desc())); // 输入：[dvoc] logits
  ...
  for (uint32_t layer = 0; layer < nlayer; layer++) {
    ...
    RUN_INFINI(infiniopGemm(desc_attn_q, workspace, workspace_size,
                            ...
                            rsrc.w_attn_q_proj[layer]->data(), 1.0, 0.0,
                            stream));
    ...
    RUN_INFINI(infiniopGemm(desc_attn_k, workspace, workspace_size,
                            ...
                            rsrc.w_attn_k_proj[layer]->data(), 1.0, 0.0,
                            stream));
    ...
    RUN_INFINI(infiniopGemm(desc_attn_v, workspace, workspace_size,
                            ...
                            rsrc.w_attn_v_proj[layer]->data(), 1.0, 0.0,
                            stream));
    ...
    for (size_t head = 0; head < nh; head++) {
      ...
      // 获取当前头的数据指针
      void *q_head_input =
          (char *)q_buf->data() + head_offset * dsize(dt_logits);
      void *q_head_output =
          (char *)q_norm_buf->data() + head_offset * dsize(dt_logits);

      // 对当前头应用归一化
      RUN_INFINI(
          infiniopRMSNorm(desc_q_norm_single, workspace, workspace_size,
    ...
    for (size_t head = 0; head < nkvh; head++) {
      ...
      // 获取当前头的数据指针
      void *k_head_input =
          (char *)k_buf->data() + head_offset * dsize(dt_logits);
      void *k_head_output =
          (char *)k_norm_buf->data() + head_offset * dsize(dt_logits);

      // 对当前头应用归一化
      RUN_INFINI(
          infiniopRMSNorm(desc_k_norm_single, workspace, workspace_size,
     ...
     */
    // o_proj
    RUN_INFINI(infiniopGemm(desc_attn_o, workspace, workspace_size,
                            logits_in->data(), o_buf->data(),
                            rsrc.w_attn_o_proj[layer]->data(), 1.0,
                            idev == 0 ? 1.0 : 0.0, stream));
    // 残差：仅 rank 0 添加原始输入
    // 如果每个设备都添加完整的残差，最终结果会是：output = partial_outputs +
    // ndev * residual (错误)
    ...
    if (rsrc.comm != nullptr) {
      RUN_INFINI(infinicclAllReduce(logits_in->data(), logits_in->data(),
                                    ntok * d, dt_logits, INFINICCL_SUM,
                                    rsrc.comm, stream));
    ...
    RUN_INFINI(infiniopGemm(desc_mlp_up, workspace, workspace_size,
                            ...
                            rsrc.w_mlp_up_proj[layer]->data(), 1.0, 0.0,
                            stream));
  ...
  if (idev == 0) {
    ...
    for (uint32_t req = 0; req < nreq; req++) {
      if (temperature[req] == 0.0f) {
        RUN_INFINI(infiniopRandomSample(
            desc_sample, workspace, workspace_size, result_buf->data(req),
            prob_buf->data(req * dvoc),
            0.0f,                       // Pass a constant, non-random value
            topp[req], topk[req], 0.0f, // Ensure temperature is 0
            stream));
      } else {
        // Probabilistic path (your original code)
        float random_val = std::uniform_real_distribution<float>(0, 1)(gen);
        RUN_INFINI(infiniopRandomSample(
            desc_sample, workspace, workspace_size, result_buf->data(req),
            prob_buf->data(req * dvoc), random_val, topp[req], topk[req],
            temperature[req], stream));
      }
    }
  ...
  for (uint32_t req = 0; req < nreq; req++) {
    ...
    infiniopDestroyCausalSoftmaxDescriptor(
        desc_qk_softmaxs[req]);                            // 因果 softmax
