...
int main(int argc, char **argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <weights.bin> [cpu|nvidia|hygon]" << std::endl;
        return 1;
    }
    std::string dev_arg = argc >= 3 ? std::string(argv[2]) : "nvidia";
    ...
    } else if (dev_arg == "nvidia" || dev_arg == "cuda") {
        device = INFINI_DEVICE_NVIDIA;
    } else if (dev_arg == "hygon") {
        device = INFINI_DEVICE_HYGON;
    } else {
        std::cerr << "Unknown device: " << dev_arg << std::endl;
        return 1;
    }
    RUN_INFINI(infinirtSetDevice(device, 0));
    ...
    auto kv = createKVCache(meta.layers, meta.seq_in, meta.heads, meta.head_dim, meta.head_dim,
                            INFINI_DTYPE_F16, device, &dev_id, 1);
