...
Environment knobs (read at install/build time):
  ...
  INFINILM_BUILD_FLASH_ATTN=1
      ...
      libflash-attn-{nvidia,hygon}.so is co-located with InfiniLM runtime libs.
...
setup(
    ...
    packages=[
        pkg
        for pkg in find_packages(where="python")
        if pkg == "infinilm" or pkg.startswith("infinilm.")
    ],
    package_data={
        "infinilm": [
            "lib/_infinilm*.so",
            "lib/libinfinirt.so",
            "lib/libinfiniccl.so",
            "lib/libflash-attn-*.so",
            "lib/libflash_attn_*.so",
        ],
    },
