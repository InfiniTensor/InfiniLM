...
INFINIOP_CACHABLE_DESCRIPTOR(Descriptor, Digamma, 100);
...
void *plan(Tensor y, const Tensor &x) {
    ...
    INFINIOP_CACHABLE_DESCRIPTOR_GET_OR_CREATE(
        Descriptor, descriptor, Digamma,
        seed,
        y->desc(), x->desc());

    INFINIOP_WORKSPACE_TENSOR(workspace, Digamma, descriptor);
...
void run(void *planned_meta) {
    ...
    INFINICORE_CHECK_ERROR(infiniopDigamma(
        p->descriptor->desc,
