...
infiniStatus_t Descriptor::calculate(
    ...
    if (reduction == Reduction::NONE) {
        ...
        if (ndim > 0) {
            ...
            CHECK_MOORE(musaMemcpyAsync(shape_d, shape.data(), ndim * sizeof(size_t), musaMemcpyHostToDevice, musa_stream));
            CHECK_MOORE(musaMemcpyAsync(input_strides_d, input_strides.data(), ndim * sizeof(ptrdiff_t), musaMemcpyHostToDevice, musa_stream));
            CHECK_MOORE(musaMemcpyAsync(target_strides_d, target_strides.data(), ndim * sizeof(ptrdiff_t), musaMemcpyHostToDevice, musa_stream));
            CHECK_MOORE(musaMemcpyAsync(output_strides_d, output_strides.data(), ndim * sizeof(ptrdiff_t), musaMemcpyHostToDevice, musa_stream));
        }
        ...
        case INFINI_DTYPE_F16: {
            ...
            CHECK_MOORE(musaMemsetAsync(accum, 0, sizeof(float), musa_stream));
            cuda::hinge_embedding_loss_reduce_kernel<half, float, BLOCK_SIZE><<<num_blocks, BLOCK_SIZE, 0, musa_stream>>>(
                accum,
                reinterpret_cast<const half *>(input),
                reinterpret_cast<const half *>(target),
                input_size,
                ...
                target_contiguous,
                margin_val);
            cuda::hinge_embedding_loss_finalize_kernel<half, float><<<1, 1, 0, musa_stream>>>(
        ...
        case INFINI_DTYPE_BF16: {
            ...
            float *accum = reinterpret_cast<float *>(base);
            CHECK_MOORE(musaMemsetAsync(accum, 0, sizeof(float), musa_stream));
            cuda::hinge_embedding_loss_reduce_kernel<cuda_bfloat16, float, BLOCK_SIZE><<<num_blocks, BLOCK_SIZE, 0, musa_stream>>>(
                accum,
                reinterpret_cast<const cuda_bfloat16 *>(input),
                reinterpret_cast<const cuda_bfloat16 *>(target),
                input_size,
                ...
                target_contiguous,
                margin_val);
            cuda::hinge_embedding_loss_finalize_kernel<cuda_bfloat16, float><<<1, 1, 0, musa_stream>>>(
        ...
        case INFINI_DTYPE_F64: {
            ...
            CHECK_MOORE(musaMemsetAsync(accum, 0, sizeof(double), musa_stream));
