...
#define __MASKED_SELECT_METAX_KERNEL_CUH__
...
INFINIOP_METAX_KERNEL maskedSelectGetMarkScanOnceKernel(
    ...
    if (index < total_elements) {
        size_t mask_offset = device::metax::indexToOffset(index, ndim, shape, mask_strides);
...
template <size_t BLOCK_SIZE>
INFINIOP_METAX_KERNEL maskedSelectScanWithStrideKernel(
...
template <size_t BLOCK_SIZE>
INFINIOP_METAX_KERNEL maskedSelectCountScanResultKernel(
...
INFINIOP_METAX_KERNEL maskedSelectGetDataKernel(
    ...
    size_t input_offset = device::metax::indexToOffset(index, ndim, shape, input_strides);
    size_t mask_offset = device::metax::indexToOffset(index, ndim, shape, mask_strides);
