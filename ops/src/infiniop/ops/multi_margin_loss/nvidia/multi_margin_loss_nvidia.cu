...
#include "multi_margin_loss_nvidia.cuh"
#include <algorithm>
#include <cstdint>

namespace op::multi_margin_loss::nvidia {
template <typename T>
...
void launch_kernel(
    ...
    op::multi_margin_loss::cuda::MultiMarginLossFunctor functor(info.p(), info.margin());
    ...
    if (reduction == 0) {
        ...
        op::multi_margin_loss::cuda::multi_margin_loss_kernel<T>
    ...
    // ------------------------------------------
        ...
        op::multi_margin_loss::cuda::multi_margin_loss_reduce_kernel<T>
            <<<grid_size, block_size, 0, cuda_stream>>>(
                acc_ptr, in_ptr, tar_ptr, w_ptr, N, C, functor, scale);
        op::multi_margin_loss::cuda::cast_float_to_t<T>
...
infiniStatus_t Descriptor::calculate(
    ...
    case INFINI_DTYPE_BF16:
        launch_kernel<cuda_bfloat16>(output, input, target, weight, workspace, _info, stream);
        break;
