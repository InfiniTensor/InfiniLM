...
// ==================================================================
// 类型转换辅助工具 (解决 MUSA 半精度类型转换问题)
// ==================================================================
template <typename T>
struct TypeCast {
    __device__ __forceinline__ static float to_float(T val) { return static_cast<float>(val); }
    __device__ __forceinline__ static T from_float(float val) { return static_cast<T>(val); }
};

template <>
struct TypeCast<half> {
    __device__ __forceinline__ static float to_float(half val) { return __half2float(val); }
    __device__ __forceinline__ static half from_float(float val) { return __float2half(val); }
};

template <>
struct TypeCast<__mt_bfloat16> {
    __device__ __forceinline__ static float to_float(__mt_bfloat16 val) { return __bfloat162float(val); }
    __device__ __forceinline__ static __mt_bfloat16 from_float(float val) { return __float2bfloat16(val); }
};
...
__global__ void triplet_margin_loss_kernel(
    ...
    if (n < N) {
        ...
        output[n] = TypeCast<T>::from_float(loss);
    }
}
...
#endif // __TRIPLET_MARGIN_LOSS_MOORE_KERNEL_H__
