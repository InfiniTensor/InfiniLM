...
infiniStatus_t Descriptor::create(
    ...
    if (red != Reduction::NONE) {
        if (dtype == INFINI_DTYPE_F16 || dtype == INFINI_DTYPE_BF16) {
            CHECK_MOORE(musaMalloc(&reduce_buffer, sizeof(float)));
        }
    }
...
infiniStatus_t Descriptor::calculate(
    ...
    if (reduction == Reduction::NONE) {
        ...
        case INFINI_DTYPE_F16: {
            ...
            CHECK_MOORE(musaMemsetAsync(accum, 0, sizeof(float), cuda_stream));
            float eps_val = static_cast<float>(eps);
        ...
        case INFINI_DTYPE_F64: {
            ...
            CHECK_MOORE(musaMemsetAsync(accum, 0, sizeof(double), cuda_stream));
            const int reduce_blocks = std::min(num_blocks, 1024);
