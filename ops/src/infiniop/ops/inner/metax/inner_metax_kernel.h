#ifndef __INNER_METAX_KERNEL_H__
#define __INNER_METAX_KERNEL_H__

template <size_t BLOCK_SIZE, typename T>
INFINIOP_METAX_KERNEL innerKernel(
    ...
    size_t out_offset = device::metax::indexToOffset(out_index, out_ndim, out_shape, out_strides);
    ...
    // T tmp = input[input_offset] * other[other_offset];
    ...
    for (size_t i = 0; i < oper_len; i++) {
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, cuda_bfloat16>) {
            ...
            // if (out_index == 0) {
            //     printf("input: %.10f\tother:%.10f\n", input[input_offset], other[other_offset]);
            // }
            T mul_a = __fmul_rn(input[input_offset], other[other_offset]);
            // if (out_index == 0) {
            //     printf("mul answer: %.10f\n", mul_a);
            //     printf("tmp val: %.10f\n", tmp);
            // }
            tmp = __fadd_rd(tmp, mul_a);
            // if (out_index == 0)
            //     printf("add answer: %.10f\n", tmp);
        }
        input_offset += input_strides[input_ndim - 1];
        other_offset += other_strides[other_ndim - 1];
    }
