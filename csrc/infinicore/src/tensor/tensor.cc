...
infini::rt::TensorView TensorImpl::view() {
    return infini::rt::TensorView{data(), shape(), dtype(), device(), strides()};
}
...
std::shared_ptr<TensorImpl> TensorImpl::empty(const Shape &shape,
    ...
    if (device.type() == Device::Type::kCpu) {
        if (pin_memory) {
            if (context::getDevice().type() == Device::Type::kCpu) {
...
Tensor TensorImpl::to_blob_() const {
    if (context::isGraphRecording()) {
        context::retainGraphMemory(data_.memory);
    }
    auto t = std::shared_ptr<TensorImpl>(new TensorImpl(shape(), strides(), dtype()));
