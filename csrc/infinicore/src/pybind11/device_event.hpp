...
inline void bind(py::module &m) {
    py::class_<DeviceEvent>(m, "DeviceEvent")
        ...
        .def(
            "record", [](DeviceEvent &event, std::uintptr_t stream) {
                event.record(reinterpret_cast<infini::rt::runtime::Stream>(stream));
            },
            "Record the event on a specific stream", py::arg("stream"))

        .def("synchronize", &DeviceEvent::synchronize, "Wait for the event to complete (blocking)")
        .def("query", &DeviceEvent::query, "Check if the event has been completed")
