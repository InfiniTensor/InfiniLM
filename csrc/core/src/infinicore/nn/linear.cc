...
Tensor BaseLinear::compute_linear(Tensor &input) const {
    ...
    case infinicore::quantization::QuantScheme::COMPRESSED_TENSOR_W8A8I8: {
        ...
        auto output = infinilm::backends::ops::linear_w8a8i8(input_contiguous->contiguous(), weight_packed_tensor, weight_scale_tensor, bias_opt);
        return output;
    }
    case infinicore::quantization::QuantScheme::AWQ_W4A16: {
        ...
        std::optional<Tensor> bias_opt = has_bias_ ? std::make_optional<Tensor>(static_cast<const Tensor &>(bias_)) : std::nullopt;
        auto output = infinilm::backends::ops::linear_w4a16_awq(input_contiguous->contiguous(), qweight, scales, qzeros, bias_opt);
        return output;
    }
    case infinicore::quantization::QuantScheme::GPTQ_W4A16_QY: {
        ...
        std::optional<Tensor> bias_opt = has_bias_ ? std::make_optional<Tensor>(static_cast<const Tensor &>(bias_)) : std::nullopt;
        auto output = infinilm::backends::ops::linear_w4a16_gptq_qy(input_contiguous->contiguous(), qweight, qzeros, scales, 0, 4);
        if (bias_opt.has_value()) {
            infinilm::backends::ops::add_(output, output, bias_opt.value()->as_strided(output->shape(), {0, 0, 1}));
        }
        return output;
    }
    default: {
        ...
        auto output = infinilm::backends::ops::linear(input_contiguous->contiguous(), weight_tensor->contiguous(), bias_opt);
        return output;
    }
    }
} // namespace infinicore::nn
...
Tensor BaseLinear::forward(Tensor &input, Tensor &residual) const {
    ...
    infinilm::backends::ops::add_(output, output, residual);

    return output;
}

void BaseLinear::process_weights_after_loading() {
    // MuP alpha is tracked here so model code can stay independent from the
    // operator backend. External InfiniOps scaling will be wired in the wrapper.
}
...
Tensor RowParallelLinear::forward(Tensor &input) const {
    ...
    if ((tp_size_ > 1) && (communicator_ != nullptr)) {
        infinilm::backends::ops::allreduce_(output, output, INFINICCL_SUM, communicator_);
    }
    return output;
}
