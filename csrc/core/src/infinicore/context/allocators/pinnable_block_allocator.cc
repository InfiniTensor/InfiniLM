...
constexpr size_t kDefaultMaxSizeClass = 256 * kMiB;
...
size_t configured_max_size_class() {
    const char *value = std::getenv("INFINILM_PINNABLE_MAX_BUCKET_MB");
    if (value == nullptr || value[0] == '\0') {
        return kDefaultMaxSizeClass;
    }
    ...
    if (errno != 0 || end == value || *end != '\0'
        || mb > std::numeric_limits<size_t>::max() / kMiB) {
        return kDefaultMaxSizeClass;
    }
...
PinnableBlockAllocator::PinnableBlockAllocator(Device device)
    ...
    // Keep the legacy bucket set by default. Large models can cap the cached
    // bucket size to avoid rounding large transient allocations up to 64/128/256 MB.
    const auto max_size_class = configured_max_size_class();
    size_classes_.erase(
        std::remove_if(size_classes_.begin(), size_classes_.end(),
                       [max_size_class](const SizeClass &cls) {
                           return cls.block_size > max_size_class;
                       }),
        size_classes_.end());
}
...
size_t PinnableBlockAllocator::mark_in_use_(void *ptr, bool in_use) {
    ...
    auto block = it->second;
    if (in_use && !block->in_use && block->in_free_list) {
        for (auto &cls : size_classes_) {
            if (block->size != cls.block_size) {
                continue;
            }
            auto free_it = std::find(cls.free_blocks.begin(), cls.free_blocks.end(), block);
            if (free_it != cls.free_blocks.end()) {
                cls.free_blocks.erase(free_it);
            }
            block->in_free_list = false;
            break;
        }
