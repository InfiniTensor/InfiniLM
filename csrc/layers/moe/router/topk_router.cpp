...
std::string router_backend_name(const std::shared_ptr<infinilm::config::ModelConfig> &model_config) {
    ...
    if (config_json.contains("moe_router_backend") && !config_json["moe_router_backend"].is_null()) {
        return config_json["moe_router_backend"].get<std::string>();
    }
    if (config_json.contains("topk_method") && !config_json["topk_method"].is_null() &&
        config_json["topk_method"].get<std::string>() == "noaux_tc") {
        return "fused_gate";
    }
    if (config_json.contains("scoring_func") && !config_json["scoring_func"].is_null()) {
        return config_json["scoring_func"].get<std::string>();
    }
    return "softmax";
}
...
TopKRouter::TopKRouter(std::shared_ptr<infinilm::config::ModelConfig> model_config,
    ...
    apply_routed_scaling_factor_on_output_ =
        model_config->get_or<bool>("apply_routed_scaling_factor_on_output", false);
    router_backend_ = parse_router_backend(router_backend_name(model_config));
    use_correction_bias_ =
        model_config->contains_non_null("e_score_correction_bias") ||
        router_backend_ == TopKRouterBackend::FusedGate ||
        model_config->get_or<bool>("moe_router_use_correction_bias", false);
...
std::tuple<infinicore::Tensor, infinicore::Tensor> TopKRouter::forward(const infinicore::Tensor &hidden_states) const {
    ...
    const infinicore::Tensor correction_bias =
        use_correction_bias_ ? static_cast<infinicore::Tensor>(e_score_correction_bias_) : infinicore::Tensor();
