...
Attention<AttnBackend>::Attention(std::shared_ptr<infinilm::config::ModelConfig> model_config,
                                  const infinicore::Device &device,
                                  size_t layer_idx,
                                  infinilm::engine::distributed::RankInfo rank_info)
    : AttentionBase(model_config, device, layer_idx, rank_info) {}

template <typename AttnBackend>
infinicore::Tensor Attention<AttnBackend>::forward(const infinicore::Tensor &hidden_states,
    ...
    auto attn_output = static_cast<const AttnBackend *>(this)->attn_calculate(q_reshaped, k_reshaped, v_reshaped, input, kv_cache);
