...
GGUFBlockQuantization::GGUFBlockQuantization(const nlohmann::json &quant_config)
    ...
    for (const auto &kv : table.items()) {
        ...
        if (kv.value().is_string()) {
            ...
            if (v != DENSE_MARK) {
                throw std::runtime_error(
                    "GGUFBlockQuantization: '" + name + "' 的取值 '" + v + "' 既不是整数 type id 也不是 \"" + DENSE_MARK + "\"");
            }
            ++n_dense;
        } else {
            ...
            if (!ggml_block(id)) {
                throw std::runtime_error(
                    "GGUFBlockQuantization: '" + name + "' 是不支持的 ggml type id=" + std::to_string(id) + "（当前支持 " + supported_types() + "；其余类型必须在打包期稠密化，不能留到运行期猜）");
            }
            ++n_blob;
        }
    ...
    }
    {
        ...
        if (!rules.is_array()) {
            throw std::runtime_error("GGUFBlockQuantization: activation_vperm 必须是数组，实际是 " + std::string(rules.type_name()));
        }
        for (const auto &j : rules) {
            ...
            for (const char *key : {"suffix", "num_k_heads", "num_v_per_k", "head_dim"}) {
                if (!j.contains(key)) {
                    throw std::runtime_error("GGUFBlockQuantization: activation_vperm 条目缺 '" + std::string(key) + "'");
                }
            }
            ...
            if (r.suffix.empty() || r.suffix.back() != '.' || !r.n_k || !r.r || !r.hd) {
                throw std::runtime_error(
                    "GGUFBlockQuantization: activation_vperm 条目不合法：suffix='" + r.suffix + "' 需以 '.' 结尾，三个维度需为正（实际 " + std::to_string(r.n_k) + "/" + std::to_string(r.r) + "/" + std::to_string(r.hd) + "）");
            }
            if (std::any_of(vperm_.begin(), vperm_.end(),
                            [&r](const ActVPerm &e) { return e.suffix == r.suffix; })) {
                throw std::runtime_error("GGUFBlockQuantization: activation_vperm 里 '" + r.suffix + "' 出现多次（同一条规则只能有一份）");
            }
...
int64_t GGUFBlockQuantization::resolve(const std::string &stem, std::string *matched_key) const {
    ...
    if (hits != 1) {
        throw std::runtime_error(
            "GGUFBlockQuantization: stem '" + describe(stem) + "' 在类型表里命中 " + std::to_string(hits) + " 个候选（期望恰好 1 个：'" + blob_key + "' 或 '" + dense_key + "'）；表共 " + std::to_string(types_.size()) + " 条");
    }
...
size_t GGUFBlockQuantization::row_bytes(size_t in_features, int64_t type_id) const {
    ...
    if (!b) {
        throw std::runtime_error(
            "GGUFBlockQuantization: 不支持的 ggml type id=" + std::to_string(type_id) + "（当前支持 " + supported_types() + "）");
    }
    if (in_features % b->block_size != 0) {
        throw std::runtime_error(
            "GGUFBlockQuantization: in_features=" + std::to_string(in_features) + " 不能被 " + b->name + " 的块大小 " + std::to_string(b->block_size) + " 整除");
    }
...
infinicore::Tensor GGUFBlockQuantization::gather_grouped_to_tiled(
    ...
    if (ndim < 2) {
        throw std::runtime_error(
            "GGUFBlockQuantization: " + name + " 的激活 rank=" + std::to_string(ndim) + "，至少要是 [..., in_features]");
    }
    ...
    if (K != want) {
        ...
        throw std::runtime_error(
            "GGUFBlockQuantization: " + name + " 的激活末维 " + std::to_string(K) + " != activation_vperm 的 num_k_heads*num_v_per_k*head_dim = " + std::to_string(want) + "（切分后的分片不能套整头置换）");
    }
...
std::vector<ParamDescriptor> GGUFBlockQuantization::get_param_layout(
    ...
    if (stem.empty()) {
        throw std::runtime_error(
            "GGUFBlockQuantization: 构造 Linear 时没有传 checkpoint stem（in=" + std::to_string(in_features) + ", out=" + std::to_string(out_features) + "）——方案 §6.1 列出的构造点必须全部补上 prefix/stem");
    }
    if (tp_size != 1 || tp_rank != 0) {
        throw std::runtime_error(
            "GGUFBlockQuantization: 暂不支持 tensor parallel（blob 的 TP 切分留待多卡阶段）：" + describe(stem));
    }
    if (bias) {
        throw std::runtime_error(
    ...
    if (stem.back() != '.') {
        if (!has_group(stem)) {
            throw std::runtime_error(
                "GGUFBlockQuantization: 融合组 stem '" + stem + "' 在类型表里没有任何 '" + stem + ".<shard>.*' 条目");
        }
        ++n_group_;
        return {};
    }
...
infinicore::Tensor GGUFBlockQuantization::forward_shard(
    ...
    if (suffix == DENSE_SUFFIX) {
        ...
        if (type_id != DENSE_BF16) {
            throw std::runtime_error(
                "GGUFBlockQuantization: " + table_key + " 的参数后缀是 " + DENSE_SUFFIX + "，但类型表给出的 ggml type id=" + std::to_string(type_id) + "（不一致）");
        }
        auto x = input->is_contiguous() ? input : input->contiguous();
    ...
    if (suffix == BLOB_SUFFIX) {
        ...
        if (alpha != 1.0F) {
            throw std::runtime_error(
                "linear_gguf: 不支持 alpha=" + std::to_string(alpha) + "（GGUF blob 路径没有缩放权重，alpha!=1 说明上层期望与实现不符）：" + table_key);
        }
        auto x = input->is_contiguous() ? input : input->contiguous();
        auto w = weight->is_contiguous() ? weight : weight->contiguous();
    ...
    throw std::runtime_error(
        "GGUFBlockQuantization: " + table_key + " 的参数后缀 '" + suffix + "' 既不是 " + DENSE_SUFFIX + " 也不是 " + BLOB_SUFFIX);
}
...
infinicore::Tensor GGUFBlockQuantization::forward(
    ...
    if (shard_stems.empty()) {
        if (params.size() != 1) {
            throw std::runtime_error(
                "GGUFBlockQuantization: " + describe(stem) + " 有 " + std::to_string(params.size()) + " 个参数却没收到 shard_stems"
    ...
    if (shard_stems.size() != params.size()) {
        throw std::runtime_error(
            "GGUFBlockQuantization: " + describe(stem) + " 有 " + std::to_string(params.size()) + " 个 shard 参数但收到 " + std::to_string(shard_stems.size()) + " 个 shard stem（内部错误：两者应在 "
    ...
    for (const auto &kv : params) {
        if (kv.first.compare(0, std::string(SHARD_PREFIX).size(), SHARD_PREFIX) != 0) {
            throw std::runtime_error(
                "GGUFBlockQuantization: 融合 Linear 的参数名 '" + kv.first + "' 不是 " + SHARD_PREFIX + "<i>.<suffix> 形式（" + describe(stem) + "）");
        }
        ...
        if (idx >= shard_stems.size()) {
            throw std::runtime_error(
                "GGUFBlockQuantization: 参数名 '" + kv.first + "' 的 shard 下标越出 shard_stems（" + describe(stem) + "）");
        }
        std::string table_key;
...
std::vector<SplitParam> GGUFBlockQuantization::split_params(
    ...
    if (result.size() != splits.size()) {
        throw std::runtime_error(
            "GGUFBlockQuantization::split_params: " + std::to_string(splits.size()) + " 个 shard 只匹配到 " + std::to_string(result.size()) + " 个参数（GGUF 融合 Linear 应走 BaseLinear::init_fused_shards）");
    }
    return result;
}
