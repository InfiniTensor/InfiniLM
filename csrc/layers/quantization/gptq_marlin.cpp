...
void GPTQMarlin::forward_(
    ...
    float /*alpha*/) const {
    auto input_contiguous = input->is_contiguous() ? input : input->contiguous();
    const size_t k = input_contiguous->shape().back();
    const size_t m = input_contiguous->numel() / k;
    auto flat_input = input_contiguous->view({m, k});
    ...
    auto workspace = get_workspace(
        flat_output, flat_input, qweight, scales, global_scales,
        qzeros, g_idx, perm);

    infinicore::op::gptq_marlin_gemm_with_workspace_(
        workspace, flat_output, flat_input, qweight, scales, global_scales,
        qzeros, g_idx, perm, marlin::UINT4B8_ID, is_k_full_, false, true,
        false);

    if (has_bias) {
        auto bias = params.at("bias");
        infinicore::op::add_(flat_output, flat_output,
                             bias->as_strided(flat_output->shape(), {0, 1}));
    }
}
