...
StaticAttentionImpl::StaticAttentionImpl(size_t num_heads, size_t head_size, float scale, size_t num_kv_heads,
                                         size_t layer_idx)
...
infinicore::Tensor StaticAttentionImpl::forward(const AttentionLayer &layer, infinicore::Tensor &q_rope,
                                                infinicore::Tensor &k_reshaped, infinicore::Tensor &v_reshaped,
    ...
    if (infinicore::quantization::KVQuantAlgo::NONE != this->kv_quant_scheme_) {
        infinilm::KVQuantUtils::quantize(k_reshaped, v_reshaped, this->kv_quant_scheme_, k_scale, v_scale);
    }
    ...
    auto [k_total, v_total] =
        do_kv_cache_update(layer, k_permuted, v_permuted, kv_cache, past_sequence_lengths.value());
    infinicore::Tensor attn_output;
    if (false) {
        // experimental nineoothed flash attention
        attn_output = infinilm::backends::ops::flash_attention(q_reshaped, k_total, v_total,
                                                               total_sequence_lengths.value(), scale_, true);
        attn_output = attn_output->permute({0, 2, 1, 3})
                          ->contiguous()
                          ->view({batch_size, seq_len, num_heads_ * head_dim_}); // [bs, seq_len, n_q_head * head_dim]
    } else {
        size_t total_seq_len =
            reinterpret_cast<int32_t *>(total_sequence_lengths.value()->to(infinicore::Device::cpu())->data())[0];
        if (infinicore::quantization::KVQuantAlgo::NONE != this->kv_quant_scheme_) {
            infinilm::KVQuantUtils::dequantize(k_total, v_total, this->kv_quant_scheme_, k_scale, v_scale, q_reshaped);
        }
        ...
        auto attn_weight =
            infinilm::backends::ops::matmul(Q, K_transposed, scale_); // [bs * n_kv_head, ng * seq_len, total_seq_len]
...
StaticAttentionImpl::do_kv_cache_update(const AttentionLayer &layer, const infinicore::Tensor key,
