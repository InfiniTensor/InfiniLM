...
#define INFINILM_QKV_LINEAR_INIT(name, q_name, k_name, v_name, ...)                       \
    name##_ = std::make_shared<infinilm::layers::linear::QKVParallelLinear>(__VA_ARGS__); \
...
#define INFINILM_GATE_UP_LINEAR_INIT(name, gate_name, up_name, ...)                           \
    name##_ = std::make_shared<infinilm::layers::linear::GateUpParallelLinear>(__VA_ARGS__);  \
