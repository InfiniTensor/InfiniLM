...
MoeMLP::MoeMLP(std::shared_ptr<infinilm::config::ModelConfig> model_config, const infinicore::Device &device) {
    ...
    case infinicore::quantization::QuantScheme::NONE: {
        INFINICORE_NN_MODULE_INIT(gate_proj, hidden_size_, moe_intermediate_size_, false, dtype, device, tp_rank,
                                  tp_size);
        INFINICORE_NN_MODULE_INIT(up_proj, hidden_size_, moe_intermediate_size_, false, dtype, device, tp_rank,
                                  tp_size);
        INFINICORE_NN_MODULE_INIT(down_proj, moe_intermediate_size_, hidden_size_, false, dtype, device, tp_rank,
                                  tp_size, rank_info.comm);
        break;
    }
    default: {
