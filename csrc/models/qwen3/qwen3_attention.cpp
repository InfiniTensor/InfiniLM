...
Qwen3Attention::Qwen3Attention(std::shared_ptr<infinilm::config::ModelConfig> model_config,
    ...
    INFINICORE_NN_MODULE_INIT(q_norm, head_dim_, rms_norm_eps, dtype_, device_);
    INFINICORE_NN_MODULE_INIT(k_norm, head_dim_, rms_norm_eps, dtype_, device_);

    infinilm::layers::attention::init_kv_cache_quant_params(register_fn, device_, kv_cache_k_scale_, kv_cache_v_scale_);
    ...
    if (enable_workspace_manager_) {
        ...
        workspace.reserve_slot("attention.qkv",
                               {config.max_num_batched_tokens, rank_qkv_output_size_}, dtype_, device_);
        workspace.reserve_slot("attention.output",
                               {config.max_num_batched_tokens, hidden_size_},
                               dtype_,
                               device_);
    }
}
...
infinicore::Tensor Qwen3Attention::forward_static_(const infinicore::Tensor &position_ids,
    ...
    if (enable_workspace_manager_) {
        auto qkv_output = infinilm::global_state::get_forward_context()
                              .workspace_manager.get_buffer("attention.qkv",
    ...
    // 7. Project output
    if (enable_workspace_manager_) {
        auto output = infinilm::global_state::get_forward_context()
                          .workspace_manager.get_buffer("attention.output",
                                                        {batch_size, seq_len, hidden_size_},
                                                        dtype_,
                                                        device_);
        o_proj_->forward_(output, attn_output);
        return output;
    }
...
infinicore::Tensor Qwen3Attention::forward_paged_(const infinicore::Tensor &position_ids,
    ...
    if (enable_workspace_manager_) {
        auto qkv_output = infinilm::global_state::get_forward_context()
                              .workspace_manager.get_buffer("attention.qkv",
                                                            {1, seq_len, rank_qkv_output_size_},
                                                            dtype_,
                                                            device_);
        std::tie(q, k, v) = qkv_proj_->forward_split_(qkv_output, hidden_states_mutable);
    } else {
