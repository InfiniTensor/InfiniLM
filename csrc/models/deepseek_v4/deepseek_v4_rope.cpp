...
void create_gpu_ropes_if_needed(DeepseekV4RoPE::Backend backend,
    ...
    if (compress_ratio > 1) {
        ...
        auto scaling = make_compress_yarn_scaling(
            config_json, rotary_dim, static_cast<float>(compress_rope_theta));
...
infinicore::Tensor DeepseekV4RoPE::forward_gpu_(const infinicore::Tensor &x,
    ...
    if (positions.size() != seq_len) {
        throw std::runtime_error("DeepseekV4RoPE: forward positions length mismatch");
    }
