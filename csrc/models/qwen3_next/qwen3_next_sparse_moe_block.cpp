...
Qwen3NextSparseMoeBlock::Qwen3NextSparseMoeBlock(std::shared_ptr<infinilm::config::ModelConfig> model_config,
    ...
    for (size_t i = 0; i < num_experts; ++i) {
        experts_.push_back(this->register_module<Qwen3NextMLP>(
            "experts." + std::to_string(i), model_config, hidden_size, moe_intermediate_size, device, rank_info));
    }

    INFINICORE_NN_MODULE_INIT(shared_expert, model_config, hidden_size, shared_expert_intermediate_size, device, rank_info);
