...
inline void bind_model_config(py::module &m) {
    py::class_<ModelConfig, std::shared_ptr<ModelConfig>>(m, "ModelConfig")
        ...
        .def_property_readonly("dtype", &ModelConfig::get_dtype, "Get data type from config (alias for get_dtype)")
        .def_property_readonly("model_type", [](const ModelConfig &self) { return self.get<std::string>("model_type"); }, "Get model type from config")
